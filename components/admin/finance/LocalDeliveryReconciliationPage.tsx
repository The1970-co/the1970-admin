...
function statusClass(status: string) {
  ...
  if (status === "DELIVERING") return "border-blue-200 bg-blue-50 text-blue-700";
...
export default function LocalDeliveryReconciliationPage() {
  ...
  const loadMeta = async () => {
    ...
    const cashLike = Array.isArray(sourceRows)
      ? sourceRows.find((s: any) => String(s?.code || s?.name || "").toUpperCase().includes("TIEN") || String(s?.code || "").toUpperCase().includes("CASH"))
      : null;
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const result = await apiJson<any>(`/finance/local-delivery-reconciliation?${params.toString()}`);
      setData(result);
    } finally {
      setLoading(false);
    }
  ...
  const markDelivered = async (row: any, collectCod: boolean) => {
    ...
    try {
      await apiJson(`/finance/local-delivery-reconciliation/${row.orderId}/delivered`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          collectCod,
          paymentSourceId: collectCod ? paymentSourceId : undefined,
          amount: collectCod ? row.needCollectAmount || row.codAmount : undefined,
          note: note || undefined,
        }),
      });
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-neutral-500">
          Dành cho Ahamove, shipper nội bộ và các hãng nội thành. Có thể xác nhận giao thành công thủ công, đồng thời ghi nhận COD vào dòng tiền.
        </p>
      </div>
      ...
      <section className="rounded-[28px] border border-neutral-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-neutral-950">Danh sách đơn nội thành</h2>
            <p className="mt-1 text-sm text-neutral-500">
              Nhân viên đối soát có thể tick giao thành công hoặc ghi nhận COD đã nhận.
            </p>
          </div>
          <button
            onClick={() => void loadData()}
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[1180px] text-left text-sm">
            ...
            <tbody>
              {rows.map((row: any) => (
                <tr key={row.shipmentId} className="border-b align-top">
                  ...
                  <td className="py-3 font-medium text-purple-700">{row.trackingCode}</td>
                  <td className="py-3">
                    <span className={`rounded-full border px-2.5 py-1 text-xs font-medium ${statusClass(row.localStatus)}`}>
                  ...
                  <td className="py-3 text-right font-medium">{currency(row.codAmount)}</td>
                  <td className="py-3 text-right">{currency(row.paidAmount)}</td>
                  <td className="py-3 text-right font-semibold">{currency(row.needCollectAmount)}</td>
                  ...
                  <td className="py-3 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        disabled={actionId === row.orderId || row.localStatus === "DELIVERED"}
...
function Stat({ title, value, ok }: { title: string; value: any; ok?: boolean }) {
  return (
    <div className="rounded-[24px] border border-neutral-200 bg-white p-5 shadow-sm">
      <p className="text-sm text-neutral-500">{title}</p>
      <p className={`mt-2 text-2xl font-semibold ${ok ? "text-emerald-600" : "text-neutral-950"}`}>
        {value}
      </p>
    </div>
  );
