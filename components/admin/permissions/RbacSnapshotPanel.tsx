...
export default function RbacSnapshotPanel() {
  ...
  const loadSnapshots = async () => {
    try {
      ...
      const data = await apiJson<Snapshot[]>(
        "/rbac-snapshots",
        { method: "GET" },
      );
      ...
      setMessage(
        error instanceof Error
          ? error.message
          : "Không tải được snapshot phân quyền.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const createSnapshot = async () => {
    ...
    try {
      ...
      await apiJson("/rbac-snapshots", {
        method: "POST",
        body: JSON.stringify({
          name: `operation-rbac-freeze-${new Date()
            .toISOString()
            .slice(0, 10)}`,
          description:
            "Snapshot phân quyền operation.the1970 hiện tại",
        }),
      });
  ...
  const restoreSnapshot = async (snapshot: Snapshot) => {
    ...
    try {
      ...
      await apiJson(
        `/rbac-snapshots/${snapshot.id}/restore`,
        { method: "POST" },
      );

      setMessage(
        "Đã rollback phân quyền. Nhân viên cần đăng nhập lại.",
      );
  ...
  const deleteSnapshot = async (snapshot: Snapshot) => {
    ...
    try {
      ...
      await apiJson(`/rbac-snapshots/${snapshot.id}`, {
        method: "DELETE",
      });

      setMessage("Đã xoá snapshot rollback.");
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-lg font-bold text-neutral-950">
            Đóng băng / rollback phân quyền
          </h3>

          <p className="mt-1 max-w-2xl text-sm text-neutral-500">
            Chốt cấu hình quyền đang chạy trên
            operation.the1970. Nếu local thao tác nhầm vào DB
            production, có thể rollback về bản đã đóng băng.
          </p>
        </div>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="rounded-xl border border-neutral-200 bg-white px-3 py-2 text-xs font-bold text-neutral-700 transition hover:bg-neutral-100 disabled:opacity-50"
                      >
                        {deletingId === snapshot.id
                          ? "Đang xoá..."
                          : "Xoá"}
