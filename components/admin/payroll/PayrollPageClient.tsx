...
function periodWorkTotals(row: PayrollPeriod) {
  const value = row as PayrollPeriod & {
    totalProductQty?: number;
    totalItems?: number;
    totalTaggedProductQty?: number;
    productQtyByBranch?: Array<{
      branchId?: string | null;
      branchName?: string | null;
      totalProducts?: number;
    }>;
    totalWorkingHours?: number;
    workingHoursByBranch?: Array<{
      branchId?: string | null;
      branchName?: string | null;
      totalHours?: number;
    }>;
  ...
  return {
    // Đồng nhất với "SP gắn tên" trong chi tiết kỳ lương.
    // Fallback totalItems giúp frontend vẫn đúng khi backend mới chưa deploy xong.
    totalProducts: n(value.totalProductQty ?? value.totalItems ?? value.totalTaggedProductQty),
    productBranches: Array.isArray(value.productQtyByBranch)
      ? value.productQtyByBranch
      : [],
    totalHours: n(value.totalWorkingHours),
    branchHours: Array.isArray(value.workingHoursByBranch)
      ? value.workingHoursByBranch
      : [],
  };
}
