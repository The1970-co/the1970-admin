...
function channelBadge(channel: OmniChannel) {
  if (channel === "FACEBOOK") {
    return <span className="flex h-4 w-4 items-center justify-center rounded-full bg-blue-600 text-[10px] font-black text-white">f</span>;
  }
  if (channel === "INSTAGRAM") {
    return <span className="flex h-4 w-4 items-center justify-center rounded-full bg-pink-500 text-[10px] font-black text-white">◎</span>;
  }
...
export default function MessagesPageClient() {
      ...
      source = openOmniInboxEventSource((event) => {
        ...
        if (event.type === "message.created") {
          ...
          if (message?.conversationId === activeId) {
            setActiveConversation((prev) => {
              if (!prev) return prev;
              const existed = prev.messages?.some((item) => item.id === message.id);
        ...
        if (
          ...
          if (conversation.id === activeId) {
            setActiveConversation((prev) => (prev ? { ...prev, ...conversation, messages: prev.messages } : conversation));
          }
        }
      });
  ...
  const selectedSummary = useMemo(() => {
    ...
    const processing = conversations.filter((item) => item.status === "PROCESSING").length;
  ...
  async function handleAssign(assigneeId: string) {
    ...
    try {
      ...
      setActiveConversation((prev) => (prev ? { ...prev, ...updated, messages: prev.messages } : updated));
      setConversations((prev) => prev.map((item) => (item.id === updated.id ? { ...item, ...updated } : item)));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Không gán được nhân viên.");
    }
  }
  ...
  async function handleCloseConversation() {
    if (!activeConversation?.id) return;
    try {
      const updated = await updateOmniConversationStatus(activeConversation.id, { status: "CLOSED" });
      setActiveConversation((prev) => (prev ? { ...prev, ...updated, messages: prev.messages } : updated));
      setConversations((prev) => prev.map((item) => (item.id === updated.id ? { ...item, ...updated } : item)));
    } catch (err) {
  ...
  async function handleAddNote() {
    ...
    try {
      const created = await createOmniConversationNote(activeConversation.id, { note });
      setNoteDraft("");
      setActiveConversation((prev) =>
  ...
  async function handleRemoveTag(tag: string) {
    if (!activeConversation?.id) return;
    const currentTags = activeConversation.tags?.map((item) => item.tag).filter((item) => item !== tag) || [];

    try {
      const updated = await updateOmniConversationTags(activeConversation.id, { tags: currentTags });
      setActiveConversation((prev) => (prev ? { ...prev, ...updated, messages: prev.messages } : updated));
      setConversations((prev) => prev.map((item) => (item.id === updated.id ? { ...item, ...updated } : item)));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#f6f6f4] text-neutral-950">
      <div className="flex min-h-screen">
        <aside className="hidden w-[248px] shrink-0 border-r border-neutral-200 bg-white xl:block">
          <div className="border-b border-neutral-200 px-6 py-7">
            <p className="text-[11px] font-semibold uppercase tracking-[0.32em] text-neutral-400">Admin Panel</p>
            <h1 className="mt-3 text-3xl font-bold tracking-tight">The 1970</h1>
            <p className="mt-1 text-sm text-neutral-500">Core operations dashboard</p>
          </div>

          <nav className="space-y-3 p-4 text-sm">
            <SidebarGroup title="Tổng quan" active={false} items={[]} />
            <SidebarGroup title="Đơn hàng" active={false} items={["Danh sách", "Tạo đơn", "POS bán tại quầy", "Đơn trả hàng"]} />
            <SidebarGroup title="Sản phẩm" active={false} items={["Danh sách", "Khuyến mại", "Danh mục", "Nhà cung cấp"]} />
            <SidebarGroup title="Kho" active={false} items={["Kho hàng", "Lịch sử kho", "Phiếu nhập", "Phiếu chuyển kho", "Kiểm kho", "Sơ đồ kho 3D"]} />
            <SidebarGroup
              title="Bán hàng đa kênh"
              active
              items={["Tin nhắn", "Bình luận", "Livestream"]}
              badge={selectedSummary.unread}
            />
            <SidebarGroup title="Tài chính" active={false} items={["Tổng quan dòng tiền", "Phiếu thu", "Phiếu chi"]} />
          </nav>
        </aside>
        ...
        <main className="min-w-0 flex-1">
          <header className="sticky top-0 z-20 flex h-20 items-center gap-4 border-b border-neutral-200 bg-white/95 px-6 backdrop-blur">
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.34em] text-neutral-400">The 1970 Operations</p>
              <div className="mt-1 flex items-center gap-3">
                ...
                >
                  ...
                  />
                  {sseStatus === "online" ? "Realtime" : sseStatus === "connecting" ? "Đang nối" : "Mất kết nối"}
                </span>
              </div>
            </div>
            ...
            <div className="mx-auto hidden w-full max-w-xl items-center gap-3 rounded-2xl border border-neutral-200 bg-white px-4 py-3 shadow-sm lg:flex">
              ...
              />
              {search ? (
                <button onClick={() => setSearch("")} className="rounded-full p-1 text-neutral-400 hover:bg-neutral-100">
                  <X className="h-4 w-4" />
                </button>
              ) : (
                <kbd className="rounded-lg border border-neutral-200 px-2 py-1 text-[11px] text-neutral-400">Ctrl K</kbd>
              )}
            </div>
            ...
            <div className="ml-auto flex items-center gap-3">
              <button className="relative rounded-full border border-neutral-200 bg-white p-3 shadow-sm">
                ...
                {selectedSummary.unread > 0 && <span className="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white" />}
              </button>
              <div className="hidden items-center gap-3 rounded-2xl border border-neutral-200 bg-white px-3 py-2 shadow-sm md:flex">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-neutral-950 text-sm font-bold text-white">A</div>
                <div>
              ...
              <div className="border-b border-neutral-200 p-5">
                <div className="mb-4 flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">Quản lý tin nhắn đa kênh</p>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => void loadList()} className="rounded-full border border-neutral-200 p-2 text-neutral-600 hover:bg-neutral-50">
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <Metric label="Đang xử lý" value={selectedSummary.processing} />
                  ...
                  {STATUS_TABS.map((item) => (
                    <button
                      ...
                      className={cx(
                        ...
                        status === item.key ? "bg-neutral-950 text-white" : "bg-neutral-100 text-neutral-600 hover:bg-neutral-200",
                      )}
                    >
                      {item.label}
                    </button>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <select
                    value={channel}
                    onChange={(event) => setChannel(event.target.value as OmniChannel | "ALL")}
              ...
              <div className="h-[calc(100%-220px)] overflow-y-auto">
                {loadingList && !conversations.length ? (
                  ...
                  <div className="p-8 text-center">
                    ...
                    <p className="mt-1 text-sm text-neutral-500">Khi Meta webhook nhận tin mới, hội thoại sẽ xuất hiện tại đây.</p>
                  </div>
                )}
              </div>
            </aside>
              ...
              {activeConversation ? (
                  ...
                  <div className="border-b border-neutral-200">
                    <div className="flex items-center justify-between p-5">
                      <div className="flex items-center gap-3">
                        <Avatar src={customerAvatar(activeConversation)} name={customerName(activeConversation)} size="lg" />
                        <div>
                          <h3 className="text-lg font-bold">{customerName(activeConversation)}</h3>
                          <div className="mt-1 flex items-center gap-2 text-xs font-semibold text-neutral-500">
                            ...
                            <span>{statusLabel(activeConversation.status)}</span>
                          </div>
                        </div>
                      </div>

                      <div className="flex items-center gap-2 text-neutral-600">
                        <button className="rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50"><UserPlus className="h-4 w-4" /></button>
                        <button className="rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50"><Tag className="h-4 w-4" /></button>
                        <button className="rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50"><Star className="h-4 w-4" /></button>
                        <button className="rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50"><Mail className="h-4 w-4" /></button>
                        <button className="rounded-full border border-neutral-200 p-2.5 hover:bg-neutral-50"><MoreHorizontal className="h-4 w-4" /></button>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 px-5 pb-4">
                      ...
                      >
                        {ASSIGNEE_OPTIONS.map((item) => (
                          <option key={item.id || "none"} value={item.id}>{item.name}</option>
                        ))}
                      </select>
                      ...
                      <button
                        onClick={() => void markOmniConversationRead(activeConversation.id)}
                        className="rounded-2xl border border-neutral-200 px-4 py-2 text-sm font-bold text-neutral-700 hover:bg-neutral-50"
                      >
                        Đã đọc
                      </button>
                  ...
                  <div className="flex-1 overflow-y-auto bg-[#fbfbfa] p-6">
                    {loadingDetail ? (
                      ...
                      <div className="space-y-5">
                        <div className="text-center text-xs font-bold text-neutral-400">Hôm nay</div>
                      ...
                      <div className="flex h-full items-center justify-center text-center">
                        <div>
                          ...
                          <p className="mt-3 font-bold text-neutral-700">Chưa có tin nhắn</p>
                          <p className="text-sm text-neutral-500">Hội thoại đã được tạo nhưng chưa có lịch sử message.</p>
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className="rounded-3xl border border-neutral-200 bg-white p-4">
                      ...
                      <div className="flex items-center justify-between border-t border-neutral-100 pt-3">
                        <div className="flex items-center gap-1 text-neutral-500">
                          <button className="rounded-full p-2 hover:bg-neutral-100"><Sparkles className="h-4 w-4" /></button>
                          <button className="rounded-full p-2 hover:bg-neutral-100"><ImageIcon className="h-4 w-4" /></button>
                          <button className="rounded-full p-2 hover:bg-neutral-100"><Paperclip className="h-4 w-4" /></button>
                        </div>
                ...
                <div className="flex h-full items-center justify-center text-center">
                  <div>
                    ...
                    <p className="text-sm text-neutral-500">Danh sách bên trái sẽ hiển thị khách từ webhook Meta.</p>
                  </div>
                </div>
              )}
            </section>
              ...
              {activeConversation ? (
                <>
                    ...
                    {["Thông tin", "Đơn hàng", "Ghi chú"].map((item, index) => (
                      <button
                        ...
                        className={cx(
                          ...
                          index === 0 ? "border-blue-600 text-blue-600" : "border-transparent text-neutral-500",
                        )}
                      >
                        {item}
                      </button>
                  ...
                  <div className="h-[calc(100%-64px)] overflow-y-auto p-5">
                    <div className="flex items-center gap-3">
                      <Avatar src={customerAvatar(activeConversation)} name={customerName(activeConversation)} size="xl" />
                      <div>
                        <h3 className="text-xl font-bold">{customerName(activeConversation)}</h3>
                    ...
                    <div className="mt-6 space-y-3 border-b border-neutral-200 pb-5">
                      <InfoRow icon={<MessageCircle className="h-4 w-4" />} label="Kênh" value={channelLabel(activeConversation.channel)} />
                      <InfoRow icon={<Phone className="h-4 w-4" />} label="SĐT" value={activeConversation.customer?.phone || "-"} />
                      <InfoRow icon={<Circle className="h-4 w-4" />} label="Địa chỉ" value={activeConversation.customer?.address || "-"} />
                      <InfoRow icon={<Clock3 className="h-4 w-4" />} label="Lần tương tác" value={formatDateTime(activeConversation.lastMessageAt)} />
                    </div>
                    ...
                    <Panel title="Nhãn">
                      ...
                      <div className="mt-3 flex gap-2">
                        ...
                        />
                        <button onClick={() => void handleAddTag()} className="rounded-2xl bg-neutral-950 px-3 py-2 text-sm font-bold text-white">
                          Thêm
                        </button>
                      </div>
                    </Panel>
                    ...
                    <Panel title="Người phụ trách">
                      ...
                      >
                        {ASSIGNEE_OPTIONS.map((item) => (
                          <option key={item.id || "none-right"} value={item.id}>{item.name}</option>
                        ))}
                      </select>
                    </Panel>
                    ...
                    <Panel title="Ghi chú nội bộ">
                      ...
                      <div className="mt-2 text-right">
                        <button onClick={() => void handleAddNote()} className="rounded-2xl bg-neutral-950 px-4 py-2 text-sm font-bold text-white">
                          Lưu
                        </button>
                      </div>

                      <div className="mt-4 space-y-2">
                        {(activeConversation.notes || []).slice(0, 5).map((note) => (
                          <div key={note.id} className="rounded-2xl bg-neutral-50 p-3">
                            <p className="text-sm text-neutral-700">{note.note}</p>
                            <p className="mt-1 text-xs text-neutral-400">
                              {note.staffName || "Admin"} · {formatDateTime(note.createdAt)}
                            </p>
                          </div>
                        ))}
                      </div>
    ...
    return (
      <button
        className={cx(
          ...
          active ? "bg-neutral-950 text-white" : "bg-white text-neutral-900 hover:bg-neutral-50",
        )}
      >
        {title}
      </button>
  ...
  return (
    <div className="rounded-3xl border border-neutral-200 bg-white p-3">
      <div className="mb-2 flex items-center justify-between px-2 pt-1 text-sm font-bold">
        <span>{title}</span>
        {!!badge && <span className="rounded-full bg-red-500 px-2 py-0.5 text-xs text-white">{badge}</span>}
      </div>
          ...
          return (
            <button
              ...
              className={cx(
                ...
                isActive ? "bg-blue-50 text-blue-700" : "text-neutral-600 hover:bg-neutral-50",
              )}
            >
              <span>{item}</span>
              {isActive && !!badge && <span className="rounded-full bg-blue-600 px-2 py-0.5 text-xs text-white">{badge}</span>}
            </button>
          );
        })}
      </div>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="shrink-0 text-xs font-medium text-neutral-400">{formatTime(item.lastMessageAt)}</span>
        </div>

        <p className="mt-1 truncate text-sm text-neutral-600">{item.lastMessageText || "Chưa có nội dung"}</p>

        <div className="mt-2 flex flex-wrap gap-1.5">
          {item.assigneeName ? (
            <span className="rounded-full bg-neutral-100 px-2 py-0.5 text-[11px] font-bold text-neutral-500">{item.assigneeName}</span>
          ) : (
            <span className="rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-bold text-amber-700">Chưa gán</span>
          )}

          {tags.slice(0, 2).map((tag) => (
            <span key={tag.id || tag.tag} className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-bold text-blue-600">
              {tag.tag}
            </span>
          ))}
        </div>
  ...
  return (
    <div className={cx("flex gap-3", isOut && "justify-end")}>
      ...
      <div className={cx("max-w-[72%]", isOut && "text-right")}>
        ...
        >
          {message.text && <p className="whitespace-pre-wrap text-left leading-6">{message.text}</p>}
        ...
        <p className="mt-1 text-xs font-medium text-neutral-400">{formatTime(message.sentAt)}</p>
      </div>
    </div>
  );
}
...
function Avatar({
  ...
  if (src) {
    return <img src={src} alt="" className={cx("shrink-0 rounded-full object-cover", sizeClass)} />;
  }

  const initial = String(name || "?").trim().charAt(0).toUpperCase();

  return (
    <div className={cx("flex shrink-0 items-center justify-center rounded-full bg-neutral-950 font-black text-white", sizeClass)}>
      {initial}
    </div>
  );
}
...
function InfoRow({
  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-9 w-9 items-center justify-center rounded-2xl border border-neutral-200 text-neutral-500">{icon}</div>
  ...
  return (
    <div className="space-y-4">
      {Array.from({ length: 5 }).map((_, index) => (
        <div key={index} className={cx("flex animate-pulse", index % 2 === 1 && "justify-end")}>
