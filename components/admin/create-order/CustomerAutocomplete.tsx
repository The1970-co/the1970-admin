...
export default function CreateOrderPageClient() {
  ...
  const handleSubmit = async (mode: CreateOrderMode) => {
    ...
    try {
      ...
      // ======================================================
      // PHÍ SHIP THU KHÁCH
      // - shippingFee là số tiền shop thu của khách.
      // - GHN fee chỉ để tham khảo/đối soát, không được ghi đè shippingFee.
      // ======================================================
      const customerShippingFee = isPickupOrder ? 0 : parseNumber(shippingFee);
      const ghnActualFee = selectedQuote ? getFeeNumber(selectedQuote) : 0;
      const submitCustomerMustPay = Math.max(
        0,
        subtotal - totalDiscount + customerShippingFee
      );
      const submitPaidAmount = effectivePaidAmount;
      const submitRemaining = Math.max(0, submitCustomerMustPay - submitPaidAmount);
      ...
      const payload = {
        ...
        // ✅ Số tiền ship thu của khách, backend dùng để cộng vào finalAmount.
        shippingFee: customerShippingFee,
        shipFee: customerShippingFee,
        deliveryFee: customerShippingFee,
        finalAmount: submitCustomerMustPay,

        shippingSnapshot: {
          // ✅ Tách rõ phí thu khách và phí GHN.
          shippingFee: customerShippingFee,
          customerShippingFee,
          ghnActualFee,
          ...
          // ✅ Backend cũ đọc 2 key này. Giữ cả 2 cặp key để không lệch payload.
      ...
      if (
        ...
        const ghnCreated = await createGhnShipment(created.id, {
          ...
          codAmount: submitRemaining > 0 ? submitRemaining : 0,
          insuranceValue: submitCustomerMustPay,
          note: note.trim() || "",
          requiredNote: mapRequiredNoteForGhn(deliveryRequirement),
