...
type FabricCostGroup={id?:string;fabricCode:string;chinaShippingCny?:number|string|null;vietnamShippingVnd?:number|string|null;note?:string|null};
...
function ReceiptForm({ receipt, suppliers, branches, staff, samples, boards, canFabricBoardLink, canSupplierIdentity, canCostView, canCostEdit, canUpload, onClose, onSaved }: { receipt:FabricReceipt|null; suppliers:Supplier[]; branches:Branch[]; staff:Staff[]; samples:any[]; boards:FabricBoard[]; canFabricBoardLink:boolean; canSupplierIdentity:boolean; canCostView:boolean; canCostEdit:boolean; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
  ...
  function fabricCostFor(code:string){return fabricCosts.find(x=>x.fabricCode===code)||{fabricCode:code,chinaShippingCny:"",vietnamShippingVnd:"",note:""}}
  ...
  const liveCostSummary=useMemo(()=>{const rate=num(form.exchangeRateToVnd),goodsCny=rolls.reduce((sum,r)=>sum+rollAmountCny(r),0),groups=fabricCodes.map(fabricCostFor),chinaShippingCny=groups.reduce((sum,x)=>sum+num(x.chinaShippingCny),0),vietnamShippingVnd=groups.reduce((sum,x)=>sum+num(x.vietnamShippingVnd),0),goodsVnd=goodsCny*rate,chinaShippingVnd=chinaShippingCny*rate,totalShippingVnd=chinaShippingVnd+vietnamShippingVnd;return{goodsCny,goodsVnd,chinaShippingCny,chinaShippingVnd,vietnamShippingVnd,totalShippingVnd,grandTotalVnd:goodsVnd+totalShippingVnd}},[rolls,fabricCosts,form.exchangeRateToVnd]);
