...
export default function SampleFabricWorkspaceClient({ defaultSection }: { defaultSection: Section }) {
  ...
  return <div className="space-y-5">
    ...
    {showSampleForm && <SampleForm sample={editingSample} boards={workspaceMeta.boards || []} staff={staff} seasons={seasons} productGroups={productGroups} canUpload={can("design_sample.upload_images")} canViewFabricLink={owner} factories={workspaceMeta.factories || []} onClose={()=>setShowSampleForm(false)} onSaved={async()=>{setShowSampleForm(false);await reload()}} />}
    {showReceiptForm && <ReceiptForm receipt={editingReceipt} suppliers={suppliers} branches={branches} samples={metaSamples} canCostView={can("fabric_receipt.cost.view") || can("fabric_receipt.cost.edit")} canCostEdit={can("fabric_receipt.cost.edit")} canUpload={can("fabric_receipt.upload_images")} onClose={()=>setShowReceiptForm(false)} onSaved={async()=>{setShowReceiptForm(false);await reload()}} />}
...
function SampleForm({ sample, boards, staff, seasons, productGroups, factories, canUpload, canViewFabricLink, onClose, onSaved }: { sample: Sample | null; boards: FabricBoard[]; staff: Staff[]; seasons: string[]; productGroups: string[]; factories: Factory[]; canUpload: boolean; canViewFabricLink: boolean; onClose: () => void; onSaved: () => void }) {
...
function ReceiptForm({ receipt, suppliers, branches, samples, canCostView, canCostEdit, canUpload, onClose, onSaved }: { receipt:FabricReceipt|null; suppliers:Supplier[]; branches:Branch[]; samples:any[]; canCostView:boolean; canCostEdit:boolean; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
