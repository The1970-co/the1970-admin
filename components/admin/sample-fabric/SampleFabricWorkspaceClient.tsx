...
type Sample = {
  ...
  materialBoardItem?: { id?:string; boardId:string; priorityRank?:number|null; sortOrder?:number; board:{id:string;name:string;description?:string|null;sortOrder?:number} } | null;
  fabricSampleColors?: Array<{id?:string;fabricSampleId?:string;colorName:string;colorCode?:string|null;receivedMeters:any;usedMeters?:number;availableMeters?:number;sortOrder?:number;isActive?:boolean;allocations?:Array<any>}>;
  fabricSampleAllocations?: Array<{id:string;designSampleId:string;fabricSampleColorId:string;meters:any;assignedAt:string;releasedAt?:string|null;fabricSampleColor?:{id:string;colorName:string;colorCode?:string|null;receivedMeters:any;fabricSample?:Partial<Sample>}}>;
...
type MaterialBoard = { id:string; name:string; description?:string|null; sortOrder?:number; createdByName?:string|null; samples?:Array<{id:string;boardId:string;designSampleId:string;priorityRank?:number|null;sortOrder?:number;designSample:Partial<Sample>}> };
type FabricSampleOption = {
  id:string; code:string; name:string; fabricSampleReceivedAt?:string|null; coverImageUrl?:string|null;
  fabricBoard?:{id:string;boardCode:string;name?:string|null;fabricCode?:string|null}|null;
  fabricSampleColors:Array<{id:string;colorName:string;colorCode?:string|null;receivedMeters:number;usedMeters:number;availableMeters:number;allocations?:Array<any>}>;
};
...
type FabricReceipt = {
  ...
  createdByName?: string | null; createdAt?: string; updatedAt: string; rolls: Roll[]; measurements: Measurement[];
...
export default function SampleFabricWorkspaceClient({ defaultSection }: { defaultSection: Section }) {
  ...
  const [sampleFormLane, setSampleFormLane] = useState<"IDEA"|"DEPLOY"|"FABRIC_SAMPLE">("IDEA");
  ...
  return <div className={defaultSection==="samples"?"space-y-3":"space-y-5"}>
    <div className={`flex flex-col xl:flex-row xl:items-end xl:justify-between ${defaultSection==="samples"?"gap-2":"gap-4"}`}>
      <div>
        <p className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-400">Sản xuất / nguyên liệu</p>
        <h1 className={`${defaultSection==="samples"?"mt-0.5 text-xl":"mt-1 text-2xl"} font-semibold tracking-tight text-neutral-950`}>{defaultSection==="measurements"?"Thư viện bảng thông số":defaultSection==="samples"?"Triển khai mẫu":"Mẫu mã & Vải"}</h1>
        {defaultSection!=="samples"&&<p className="mt-1 text-sm text-neutral-500">{defaultSection==="measurements"?"Lưu bảng size chuẩn, nhập nhanh từ Excel hoặc ảnh và dùng lại cho các mẫu.":"Bảng vải, lịch sử gửi làm mẫu, tiến độ mẫu và kiểm thực nhận vải."}</p>}
      </div>
      {defaultSection!=="samples"&&<div className="flex flex-wrap gap-2">
        ...
        {canViewSamples && <Link href="/design-samples" className="rounded-2xl border border-neutral-300 bg-white px-4 py-2.5 text-sm font-semibold text-neutral-800">Triển khai mẫu</Link>}
    ...
    <Card className={defaultSection==="samples"?"p-3":"p-4"}><div className={`flex flex-col md:flex-row md:items-center ${defaultSection==="samples"?"gap-2":"gap-3"}`}>
      ...
      <div className="md:ml-auto flex gap-2">
        ...
        {defaultSection === "samples" && can("design_sample.create") && <button onClick={()=>{setEditingSample(null);setSampleFormLane("IDEA");setShowSampleForm(true)}} className="rounded-2xl bg-neutral-950 px-4 py-2.5 text-sm font-semibold text-white">+ Tạo mẫu</button>}
    ...
    {loading ? <Card className="p-10 text-center text-sm text-neutral-500">Đang tải dữ liệu...</Card> : defaultSection === "measurements" ? <MeasurementLibrary rows={measurementTemplates.filter(x=>(!q.trim()||[x.name,x.category,x.sourceFileName].some(v=>String(v||"").toLowerCase().includes(q.trim().toLowerCase())))&&(!measurementCategory||x.category===measurementCategory))} onEdit={setMeasurementEdit} onChanged={rows=>{setMeasurementTemplates(rows);saveMeasurementTemplates(rows)}}/> : defaultSection === "library" ?
      ...
      defaultSection === "samples" ? <SamplesView rows={filteredSamples} factories={workspaceMeta.factories || []} can={can} onCreate={lane=>{setEditingSample(null);setSampleFormLane(lane);setShowSampleForm(true)}} onEdit={x=>{setEditingSample(x);setSampleFormLane((samplePriorityLane(x) as any)||"IDEA");setShowSampleForm(true)}} onDispatch={x=>setDispatchSample(x)} onChanged={reload}/> :
      <FabricView rows={filteredReceipts} can={can} owner={owner} onEdit={x=>{setEditingReceipt(x);setShowReceiptForm(true)}} onMeasure={setMeasureReceipt} onChanged={reload}/>}
    ...
    {showSampleForm && <SampleForm sample={editingSample} initialLane={sampleFormLane} measurementTemplates={measurementTemplates} boards={workspaceMeta.boards || []} staff={staff} seasons={seasons} productGroups={productGroups} canUpload={can("design_sample.upload_images")} canViewFabricLink={owner} factories={workspaceMeta.factories || []} samplePeople={workspaceMeta.samplePeople || []} onPeopleChanged={people=>setWorkspaceMeta(m=>({...m,samplePeople:people}))} onClose={()=>setShowSampleForm(false)} onSaved={async()=>{setShowSampleForm(false);await reload()}} />}
    {showReceiptForm && <ReceiptForm receipt={editingReceipt} suppliers={suppliers} branches={branches} staff={staff} samples={metaSamples} products={workspaceMeta.products || []} boards={workspaceMeta.boards || []} canFabricBoardLink={can("fabric_receipt.fabric_board_link")} canSupplierIdentity={can("fabric_receipt.supplier_identity.view")} canCostView={owner} canCostEdit={owner} canUpload={can("fabric_receipt.upload_images")} onClose={()=>setShowReceiptForm(false)} onSaved={async()=>{setShowReceiptForm(false);await reload()}} />}
...
function SamplesView({ rows, factories, can, onCreate, onEdit, onDispatch, onChanged }: { rows: Sample[]; factories:Factory[]; can: (k:string)=>boolean; onCreate:(lane:"IDEA"|"DEPLOY"|"FABRIC_SAMPLE")=>void; onEdit:(x:Sample)=>void; onDispatch:(x:Sample)=>void; onChanged:()=>Promise<void> }) {
  ...
  async function loadMaterialBoards(){
    try{setMaterialBoards(await api<MaterialBoard[]>("/sample-fabric/samples/material-boards"))}
  ...
  const stats = useMemo(()=>({
    total: rows.filter(x=>samplePriorityLane(x)!=="FABRIC_SAMPLE").length,
    idea: rows.filter(x=>samplePriorityLane(x)==="IDEA").length,
    deploy: rows.filter(x=>samplePriorityLane(x)==="DEPLOY").length,
    fabricSample: rows.filter(x=>samplePriorityLane(x)==="FABRIC_SAMPLE").length,
    approved: rows.filter(x=>samplePriorityLane(x)==="DEPLOY"&&["APPROVED_FOR_PRODUCTION","IN_PRODUCTION","COMPLETED"].includes(x.status)).length,
  }),[rows]);
  ...
  const unassignedIdeaCount=rows.filter(x=>samplePriorityLane(x)==="IDEA"&&!rowBoardIds(x).length).length;

  const visible=useMemo(()=>{
    const next=rows.filter(row=>{
      const inTab=samplePriorityLane(row)===tab;
  ...
  const sectionGroups=useMemo(()=>{
    if(sectionMode==="MATERIAL"){
      ...
      const groups=materialBoards.map(board=>{
        const items=(board.samples||[])
          .filter(x=>visibleIds.has(String(x.designSampleId)))
          .map(x=>{
            const row=visible.find(v=>v.id===x.designSampleId);
            if(row)assigned.add(row.id);
            return row?{row,priorityRank:x.priorityRank??null,sortOrder:x.sortOrder??0}:null;
          })
          .filter(Boolean) as Array<{row:Sample;priorityRank:number|null;sortOrder:number}>;
        items.sort((a,b)=>{
          const ar=a.priorityRank??Number.MAX_SAFE_INTEGER, br=b.priorityRank??Number.MAX_SAFE_INTEGER;
          if(ar!==br)return ar-br;
    ...
    if(sectionMode==="FACTORY"){
      const factoryById=new Map(factories.map(f=>[String(f.id),f]));
      ...
      visible.forEach(row=>{
        ...
        const factoryName=String(factory?.name||row.sampleFactoryName||"Chưa chọn nhà may").trim()||"Chưa chọn nhà may";
        const factoryCode=String(factory?.code||"").trim();
        const id=factory?.id?String(factory.id):factoryName==="Chưa chọn nhà may"?"__UNASSIGNED_FACTORY__":`factory-name-${factoryName}`;
        const label=factoryCode?`${factoryCode} · ${factoryName}`:factoryName;
      ...
      return Array.from(groups.values())
        .sort((a,b)=>{
          const ae=a.id==="__UNASSIGNED_FACTORY__"?1:0, be=b.id==="__UNASSIGNED_FACTORY__"?1:0;
          if(ae!==be)return ae-be;
          return a.name.localeCompare(b.name,"vi",{numeric:true,sensitivity:"base"});
        })
        .map((group,index)=>({
    ...
    const map=new Map<string,Sample[]>();
    visible.forEach(row=>{
      const key=sampleSectionGroup(row,sectionMode as "MATERIAL"|"CATEGORY"|"FABRIC");
  ...
  async function saveMaterialBoard(){
    ...
    try{
      ...
      setMaterialBoardForm(null);
      await loadMaterialBoards();
    }catch(e){window.alert(e instanceof Error?e.message:"Không lưu được bảng chất liệu.")}
    finally{setMaterialBusy(false)}
  }

  async function deleteMaterialBoard(board:MaterialBoard){
    if(!window.confirm(`Xoá bảng chất liệu "${board.name}"? Mẫu bên trong sẽ chuyển về Chưa phân bảng chất liệu.`))return;
    try{
      setMaterialBusy(true);
      await api(`/sample-fabric/samples/material-boards/${board.id}`,{method:"DELETE"});
      await Promise.all([loadMaterialBoards(),onChanged()]);
    }catch(e){window.alert(e instanceof Error?e.message:"Không xoá được bảng chất liệu.")}
    finally{setMaterialBusy(false)}
  }

  async function moveToMaterialBoard(row:Sample,boardId:string){
    try{
      await api(`/sample-fabric/samples/${row.id}/material-board`,{method:"PATCH",body:JSON.stringify({boardId:boardId||null})});
      await Promise.all([loadMaterialBoards(),onChanged()]);
    }catch(e){window.alert(e instanceof Error?e.message:"Không chuyển được bảng chất liệu.")}
  }

  async function moveMaterialRelative(row:Sample,direction:-1|1){
  ...
  async function setMaterialPriority(row:Sample,boardId:string,rank:number|null){
    ...
    try{
      ...
      await Promise.all([loadMaterialBoards(),onChanged()]);
    }catch(e){window.alert(e instanceof Error?e.message:"Không cập nhật được STT trong bảng chất liệu.")}
  }

  const featured=visible.find(x=>x.id===featuredId)||visible[0]||null;
  const featuredImages=featured?sampleVisuals(featured):[];

  async function refreshBoardsAndSamples(){await Promise.all([loadIdeaBoards(),loadMaterialBoards(),onChanged()])}
  ...
  async function moveSample(row:Sample,target:"IDEA"|"DEPLOY"){
    if(!can("design_sample.edit"))return;
    try{
      const payload:any={priorityLane:target,priorityRank:null};
      if(target==="IDEA")payload.status="IDEA";
      else if(target==="DEPLOY"&&String(row.status||"IDEA")==="IDEA")payload.status="FABRIC_SELECTED";
      await api(`/sample-fabric/samples/${row.id}`,{method:"PATCH",body:JSON.stringify(payload)});
      ...
      <div className="flex gap-4 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 grid gap-y-1 text-xs text-neutral-600">
            <div>{samplePriorityLane(row)==="FABRIC_SAMPLE"?<>
  Nhận vải mẫu: <b>{date(row.fabricSampleReceivedAt)||"—"}</b>
  {row.fabricSampleColors?.length?<> · <b>{row.fabricSampleColors.length} màu</b> · còn <b>{fmt(row.fabricSampleColors.reduce((sum,c)=>sum+Math.max(0,num(c.receivedMeters)-(c.allocations||[]).filter((a:any)=>!a.releasedAt).reduce((s:number,a:any)=>s+num(a.meters),0)),0),3)}m</b></>:null}
</>:<>Tạo: <b>{sampleCreatedLabel(row.createdAt)}</b></>} · {sampleParentCategory(row.category)} / <b>{row.category||"Chưa phân loại"}</b></div>
      ...
      <div className="border-t border-neutral-100 px-4 py-3">
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            {can("design_sample.edit")&&tab==="IDEA"&&<button type="button" onClick={()=>void moveSample(row,"DEPLOY")} className="rounded-xl border px-3 py-2 text-xs font-semibold">Chuyển sang triển khai →</button>}
            {can("design_sample.edit")&&tab==="DEPLOY"&&<button type="button" onClick={()=>void moveSample(row,"IDEA")} className="rounded-xl border px-3 py-2 text-xs font-semibold">← Đưa về ý tưởng</button>}
  ...
  return <div className="relative overflow-hidden rounded-[28px]">
    {pageBackgroundUrl&&<div className="pointer-events-none absolute inset-0" style={{backgroundImage:`linear-gradient(rgba(255,255,255,${1-backgroundVisibility/100}),rgba(255,255,255,${1-backgroundVisibility/100})), url("${assetUrl(pageBackgroundUrl)}")`,backgroundSize:"cover",backgroundPosition:"center",backgroundRepeat:"no-repeat"}}/>}
    <div className="relative z-[1] space-y-3">
    <div className="grid grid-cols-2 gap-2 lg:grid-cols-5">{[["Tổng mẫu",stats.total],["Ý tưởng",stats.idea],["Đang triển khai",stats.deploy],["Vải mẫu",stats.fabricSample],["Đã duyệt SX",stats.approved]].map(([l,v])=><Card key={l} className="px-3 py-2.5"><div className="flex items-center justify-between gap-3"><div className="text-[11px] font-semibold uppercase tracking-wide text-neutral-400">{l}</div><div className="text-lg font-semibold">{v}</div></div></Card>)}</div>

    <Card className="overflow-hidden">
      <div className="grid grid-cols-3 border-b bg-neutral-50 p-1">
        <button type="button" onClick={()=>{setTab("IDEA");setFeaturedId("")}} className={`rounded-xl px-4 py-2 text-sm font-semibold ${tab==="IDEA"?"bg-neutral-950 text-white":"text-neutral-500"}`}>Ý tưởng · {stats.idea}</button>
        ...
        <button type="button" onClick={()=>{setTab("FABRIC_SAMPLE");setFeaturedId("");setBoardFilter("")}} className={`rounded-xl px-4 py-2 text-sm font-semibold ${tab==="FABRIC_SAMPLE"?"bg-neutral-950 text-white":"text-neutral-500"}`}>Vải mẫu · {stats.fabricSample}</button>
      </div>
      {can("design_sample.create")&&<div className="flex justify-end border-b bg-white px-3 py-2">
        <button type="button" onClick={()=>onCreate(tab)} className="rounded-xl bg-neutral-950 px-3 py-2 text-[11px] font-semibold text-white">{tab==="FABRIC_SAMPLE"?"+ Tạo vải mẫu":"+ Tạo mẫu"}</button>
      </div>}
      ...
      {tab==="IDEA"&&<div className="border-b bg-white px-3 py-2.5">
        ...
        <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1">
          ...
          {ideaBoards.map(board=>{
            ...
            const ideaCount=linked.filter((x:any)=>samplePriorityLane(x)==="IDEA").length;
            const deployed=linked.filter((x:any)=>samplePriorityLane(x)==="DEPLOY").length;
      ...
      <div className="flex flex-wrap items-center gap-2 px-3 py-2.5">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          {pageBackgroundUrl&&<div className="flex h-9 items-center gap-2 rounded-xl border bg-white/90 px-2.5">
            <span className="text-[10px] font-semibold text-neutral-500">Độ rõ {backgroundVisibility}%</span>
            <input type="range" min="10" max="100" step="5" value={backgroundVisibility} onChange={e=>changeBackgroundVisibility(Number(e.target.value))} className="w-24"/>
          </div>}
      ...
      <Card className="h-fit overflow-hidden xl:sticky xl:top-4">
        {featured?<><SampleImageStack row={featured} large/><div className="p-4">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {can("design_sample.edit")&&tab==="IDEA"&&<button type="button" onClick={()=>void moveSample(featured,"DEPLOY")} className="rounded-xl bg-neutral-950 px-3 py-2 text-xs font-semibold text-white">Chuyển sang triển khai</button>}{can("design_sample.edit")&&tab==="DEPLOY"&&<button type="button" onClick={()=>void moveSample(featured,"IDEA")} className="rounded-xl border px-3 py-2 text-xs font-semibold">Đưa về ý tưởng</button>}
          </div>
      ...
      <div className="flex min-w-max items-start gap-3">
        {sectionGroups.map(group=><section key={group.id||group.name} className="w-[290px] shrink-0 overflow-hidden rounded-2xl border border-neutral-200 bg-white/92 shadow-sm backdrop-blur-sm">
          <div className="border-b bg-white/95 px-3 py-2.5 backdrop-blur">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0"><h3 className="truncate text-sm font-semibold">{group.name}</h3><div className="mt-0.5 text-[10px] text-neutral-400">{group.rows.length} mẫu{yearFilter?` · ${yearFilter}`:""}</div></div>
              {sectionMode==="MATERIAL"&&group.id!=="__UNASSIGNED_MATERIAL__"&&can("design_sample.edit")&&<div className="flex gap-1">
                <button type="button" onClick={()=>setMaterialBoardForm({id:group.id,name:group.name,description:group.description||"",sortOrder:group.sortOrder||0})} className="rounded-lg border px-2 py-1 text-[9px] font-semibold">Sửa</button>
                <button type="button" onClick={()=>{const b=materialBoards.find(x=>x.id===group.id);if(b)void deleteMaterialBoard(b)}} className="rounded-lg border border-red-200 px-2 py-1 text-[9px] font-semibold text-red-600">Xoá</button>
              </div>}
            </div>
          </div>
    ...
    {materialBoardForm&&<Modal title={materialBoardForm.id?"Sửa bảng chất liệu":"Tạo bảng chất liệu"} onClose={()=>setMaterialBoardForm(null)}>
      <div className="space-y-4 p-5">
        <Field label="Tên bảng chất liệu"><input autoFocus className={inputClass} value={materialBoardForm.name} onChange={e=>setMaterialBoardForm(x=>x?{...x,name:e.target.value}:x)} placeholder="VD: Da lộn, Canvas wax, Kaki nhẹ..."/></Field>
        <Field label="Mô tả"><textarea className={`${inputClass} min-h-24`} value={materialBoardForm.description} onChange={e=>setMaterialBoardForm(x=>x?{...x,description:e.target.value}:x)} placeholder="Ghi chú nhóm chất liệu nếu cần."/></Field>
        <Field label="Thứ tự cột"><input type="number" className={inputClass} value={materialBoardForm.sortOrder??0} onChange={e=>setMaterialBoardForm(x=>x?{...x,sortOrder:Number(e.target.value)}:x)}/></Field>
        <div className="flex justify-end gap-2 border-t pt-4"><button type="button" onClick={()=>setMaterialBoardForm(null)} className="rounded-xl border px-4 py-2 text-sm">Đóng</button><button type="button" disabled={materialBusy||!materialBoardForm.name.trim()} onClick={()=>void saveMaterialBoard()} className="rounded-xl bg-neutral-950 px-4 py-2 text-sm font-semibold text-white disabled:opacity-40">{materialBusy?"Đang lưu...":"Lưu bảng"}</button></div>
      </div>
    </Modal>}
...
function receiptLegacyPriceCny(receipt:FabricReceipt){
  const price=num(receipt.unitPrice);
  const priceVnd=num(receipt.unitPriceVnd);
  const currency=String(receipt.priceCurrency||"VND").toUpperCase();
  const rate=num(receipt.exchangeRateToVnd);
  if(currency==="CNY"&&price>0)return price;
  if(rate>0&&priceVnd>0)return priceVnd/rate;
  if(currency==="VND"&&rate>0&&price>0)return price/rate;
  return 0;
}
function receiptLegacyCostQty(receipt:FabricReceipt,roll?:Roll){
  const unit=String(receipt.priceUnit||"METER").toUpperCase();
  if(unit==="ROLL")return roll?1:Math.max(1,(receipt.rolls||[]).length||num(receipt.rollCount));
  if(unit==="KG")return roll?(num(roll.supplierDeclaredKg)||num(roll.actualKg)):(receiptRollTotal(receipt,"supplierDeclaredKg")||receiptRollTotal(receipt,"actualKg"));
  return roll?(num(roll.supplierDeclaredM)||num(roll.actualM)):(receiptRollTotal(receipt,"supplierDeclaredM")||receiptRollTotal(receipt,"actualM"));
}
function receiptCostSummaryForDisplay(receipt:FabricReceipt):ReceiptCostSummary{
  const server=receipt.costSummary;
  const rate=num(receipt.exchangeRateToVnd)||num(server?.exchangeRateToVnd);
  const rollGoodsCny=(receipt.rolls||[]).reduce((sum,r)=>sum+rollAmountCny(r),0);
  const legacyUnit=num(receipt.unitPrice); const legacyUnitVnd=num(receipt.unitPriceVnd);
  const legacyCurrency=String(receipt.priceCurrency||"VND").toUpperCase();
  const legacyQty=receiptLegacyCostQty(receipt);
  const legacyGoodsCny=legacyCurrency==="CNY"?legacyUnit*legacyQty:0;
  const legacyGoodsVnd=legacyUnitVnd>0?legacyUnitVnd*legacyQty:(legacyCurrency==="VND"?legacyUnit*legacyQty:legacyGoodsCny*rate);
  const goodsCny=rollGoodsCny>0?rollGoodsCny:(num(server?.goodsCny)>0?num(server?.goodsCny):legacyGoodsCny);
  const goodsVnd=goodsCny>0&&rate>0?goodsCny*rate:(num(server?.goodsVnd)>0?num(server?.goodsVnd):legacyGoodsVnd);
  const chinaShippingCny=(receipt.fabricCosts||[]).reduce((s,c)=>s+num(c.chinaShippingCny),0) || num(server?.chinaShippingCny);
  const chinaShippingVnd=chinaShippingCny>0&&rate>0?chinaShippingCny*rate:num(server?.chinaShippingVnd);
  const totalKg=receiptRollTotal(receipt,"actualKg")||receiptRollTotal(receipt,"supplierDeclaredKg");
  const vietnamShippingVnd=(receipt.fabricCosts||[]).reduce((s,c)=>{
    const stored=num(c.vietnamShippingVnd);
    const perKg=num(c.vietnamShippingRateVndPerKg);
    return s+(stored>0?stored:(perKg>0?perKg*totalKg:0));
  },0) || num(server?.vietnamShippingVnd);
...
function printFabricReceipt(receipt:FabricReceipt,showSupplierIdentity:boolean){
  ...
  const fabricCodeText=parseFabricCodes(receipt.fabricCode||receipt.fabricBoardCode||"").join("  ·  ")||"—";
  ...
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Phiếu nhận vải ${printHtmlEscape(receipt.receiptCode)}</title><style>@page{size:A4 portrait;margin:14mm}*{box-sizing:border-box}html,body{max-width:100%;margin:0}body{font-family:Arial,sans-serif;color:#111;font-size:11.5px;background:#fff}.page{width:100%;max-width:190mm;margin:0 auto;padding:10mm 8mm}h1{font-size:20px;margin:0 0 3px}.sub{color:#555}.top{display:flex;justify-content:space-between;gap:24px;border-bottom:2px solid #111;padding-bottom:12px}.meta{display:grid;grid-template-columns:1fr 1fr;gap:8px 28px;margin:16px 0}.meta div{border-bottom:1px dotted #aaa;padding:5px 0}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:12px 0}.box{border:1px solid #bbb;padding:8px}.box small{display:block;color:#666;text-transform:uppercase;font-size:9px;margin-bottom:4px}table{width:100%;max-width:100%;table-layout:fixed;border-collapse:collapse;margin-top:12px}th,td{border:1px solid #999;padding:6px;vertical-align:top;overflow-wrap:anywhere;word-break:break-word}th{background:#f2f2f2;text-align:left;font-size:10px;text-transform:uppercase}.num{text-align:right}td span{color:#666;font-size:10px}.sign{display:grid;grid-template-columns:1fr 1fr;gap:100px;margin-top:42px;text-align:center}.sign div{padding-top:8px;border-top:1px solid #111;font-weight:bold}.note{margin-top:14px;padding:10px;border:1px solid #bbb;min-height:48px}.print-actions{margin-bottom:14px;text-align:right}@media print{body{font-size:10.5px}.page{max-width:none;padding:0;margin:0}.print-actions{display:none}th,td{padding:5px}}</style></head><body><div class="page"><div class="print-actions"><button onclick="window.print()">In phiếu</button></div><div class="top"><div><h1>THE 1970 · PHIẾU NHẬN VẢI</h1><div class="sub">Mã phiếu: <b>${printHtmlEscape(receipt.receiptCode)}</b></div></div><div style="text-align:right"><b>Ngày nhận: ${printHtmlEscape(receivedDate)}</b><br><span class="sub">Ngày in: ${printHtmlEscape(new Date().toLocaleString("vi-VN"))}</span></div></div><div class="meta"><div>Người nhận: <b>${printHtmlEscape(receipt.receivedByName||"—")}</b></div><div>Kho nhận: <b>${printHtmlEscape(receipt.branch?.name||"—")}</b></div><div>Nhà cung cấp: <b>${printHtmlEscape(supplier)}</b></div><div>Mẫu / sản phẩm: <b>${printHtmlEscape(receipt.designSample?`${receipt.designSample.code} · ${receipt.designSample.name}`:receipt.product?`${receipt.product.slug} · ${receipt.product.name}`:"—")}</b></div><div>Tên vải: <b>${printHtmlEscape(receipt.fabricName||"—")}</b></div><div>Tên Mã Vải: <b>${printHtmlEscape(fabricCodeText)}</b></div><div>Màu: <b>${printHtmlEscape(receipt.colorName||"—")} ${printHtmlEscape(receipt.colorCode||"")}</b></div><div>Lô: <b>${printHtmlEscape(receipt.lotCode||"—")}</b></div></div><div class="summary"><div class="box"><small>Số cây</small><b>${rolls.length}</b></div><div class="box"><small>NCC báo</small><b>${printHtmlEscape(fmt(declaredM,3))} m</b><br>${printHtmlEscape(fmt(declaredKg,3))} kg</div><div class="box"><small>Thực nhận</small><b>${printHtmlEscape(fmt(actualM,3))} m</b><br>${printHtmlEscape(fmt(actualKg,3))} kg</div><div class="box"><small>Chênh lệch</small><b>${actualM-declaredM>0?"+":""}${printHtmlEscape(fmt(actualM-declaredM,3))} m</b><br>${actualKg-declaredKg>0?"+":""}${printHtmlEscape(fmt(actualKg-declaredKg,3))} kg</div></div><table><thead><tr><th>STT</th><th>Mã vải / cây</th><th>Màu</th><th>NCC báo</th><th>Thực nhận</th><th>GSM</th><th>Ghi chú</th></tr></thead><tbody>${rowsHtml||'<tr><td colspan="7" style="text-align:center">Chưa có cây vải</td></tr>'}</tbody></table>${receipt.note?`<div class="note"><b>Ghi chú:</b> ${printHtmlEscape(receipt.note)}</div>`:""}<div class="sign"><div>NGƯỜI GIAO VẢI</div><div>NGƯỜI NHẬN VẢI<br><span style="font-weight:normal">${printHtmlEscape(receipt.receivedByName||"")}</span></div></div></div></body></html>`);
  w.document.close();w.focus();
}
function FabricReceiptQuickView({receipt,can,canSeeCost,onClose}:{receipt:FabricReceipt;can:(k:string)=>boolean;canSeeCost:boolean;onClose:()=>void}) {
  const declaredM=receiptRollTotal(receipt,"supplierDeclaredM"),declaredKg=receiptRollTotal(receipt,"supplierDeclaredKg");
  const actualM=receiptRollTotal(receipt,"actualM"),actualKg=receiptRollTotal(receipt,"actualKg");
  const dm=actualM-declaredM,dkg=actualKg-declaredKg;
  const cost=receiptCostSummaryForDisplay(receipt);
  const legacyUnitCny=receiptLegacyPriceCny(receipt);
  const rolls=[...(receipt.rolls||[])].sort((a,b)=>(num(a.sortOrder)-num(b.sortOrder))||String(a.fabricCode||"").localeCompare(String(b.fabricCode||""),"vi",{numeric:true}));
  return <Modal title={`${receipt.receiptCode} · Xem nhanh cây vải`} onClose={onClose} wide>
    <div className="space-y-4 p-5">
      ...
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <Card className="p-3"><div className="text-[10px] font-semibold uppercase text-neutral-400">Nhập kho / kiểm tra</div><div className="mt-1 font-semibold">{fmt(declaredM,3)} m NCC</div><div className="text-xs text-neutral-500">Quy đổi kiểm tra {fmt(actualM,3)} m · cân {fmt(actualKg,3)} kg · {rolls.length} cây</div></Card>
        ...
        <Card className="p-3"><div className="text-[10px] font-semibold uppercase text-neutral-400">Ngày / người nhận</div><div className="mt-1 font-semibold">{receipt.receivedAt?date(receipt.receivedAt):"—"}</div><div className="text-xs text-neutral-500">{receipt.receivedByName||"—"}{receipt.branch?.name?` · ${receipt.branch.name}`:""}</div></Card>
      </div>

      {canSeeCost&&<div className="space-y-2">
        <div className="grid gap-3 md:grid-cols-4">
          <Card className="p-3"><div className="text-[10px] font-semibold uppercase text-neutral-400">Tiền vải</div><div className="mt-1 font-semibold">{money(cost.goodsVnd)}</div><div className="text-xs text-neutral-500">{cost.goodsCny>0?`${fmt(cost.goodsCny,2)} CNY${cost.exchangeRateToVnd>0?` · tỷ giá ${fmt(cost.exchangeRateToVnd,0)}`:""}`:"Giá lưu theo phiếu"}</div></Card>
          <Card className="p-3"><div className="text-[10px] font-semibold uppercase text-neutral-400">Ship China</div><div className="mt-1 font-semibold">{money(cost.chinaShippingVnd)}</div><div className="text-xs text-neutral-500">{cost.chinaShippingCny>0?`${fmt(cost.chinaShippingCny,0)} CNY`:"—"}</div></Card>
          <Card className="p-3"><div className="text-[10px] font-semibold uppercase text-neutral-400">Ship Việt Nam</div><div className="mt-1 font-semibold">{money(cost.vietnamShippingVnd)}</div></Card>
          <Card className="bg-neutral-950 p-3 text-white"><div className="text-[10px] font-semibold uppercase text-neutral-400">Tổng đơn</div><div className="mt-1 text-lg font-semibold">{money(cost.grandTotalVnd)}</div><div className="text-xs text-neutral-400">Đã gồm vận chuyển</div></Card>
        </div>
        {(num(receipt.unitPrice)>0||cost.exchangeRateToVnd>0)&&<div className="rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-900"><b>Dữ liệu giá đã lưu:</b> {num(receipt.unitPrice)>0?`${fmt(receipt.unitPrice,2)} ${String(receipt.priceCurrency||"VND").toUpperCase()}/${receipt.priceUnit==="KG"?"kg":receipt.priceUnit==="ROLL"?"cây":"m"}`:"—"}{cost.exchangeRateToVnd>0?` · Tỷ giá ${fmt(cost.exchangeRateToVnd,0)} VND/CNY`:""}</div>}
      </div>}
          ...
          {rolls.map((r,i)=>{
            ...
            return <div key={r.id||i} className="border-b p-4 last:border-b-0">
              <div className="grid gap-3 lg:grid-cols-[70px_1.1fr_1fr_1fr_1.3fr]">
                ...
                <div><div className="text-[10px] font-semibold uppercase text-neutral-400">Số lượng / GSM</div><div className="mt-1 font-semibold">Nhập kho NCC {fmt(r.supplierDeclaredM,3)} m · {fmt(r.supplierDeclaredKg,3)} kg</div><div className="text-xs text-neutral-500">Kiểm tra: {fmt(r.actualM,3)} m quy đổi · {fmt(r.actualKg,3)} kg cân</div><div className="mt-1 text-xs font-semibold">{r.measuredGsm?`${fmt(r.measuredGsm,1)} GSM`:"Chưa đo GSM cây"}</div></div>
                <div>
                  ...
                  {canSeeCost&&<div className="mt-1 text-xs text-neutral-500">{(()=>{const unit=num(r.unitPriceCny)>0?num(r.unitPriceCny):legacyUnitCny;const priceUnit=r.priceUnit||receipt.priceUnit||"METER";const legacyQty=receiptLegacyCostQty(receipt,r);const usedQty=num(r.unitPriceCny)>0?qty:legacyQty;const usedAmount=num(r.unitPriceCny)>0?amount:unit*usedQty;return unit>0?`${fmt(unit,2)} CNY/${priceUnit==="KG"?"kg":priceUnit==="ROLL"?"cây":"m"} · SL tính giá ${fmt(usedQty,3)} · ${fmt(usedAmount,2)} CNY${num(r.unitPriceCny)>0?"":" · giá phiếu cũ"}`:"Chưa có giá cây";})()}</div>}
                </div>
              </div>
...
function FabricView({ rows, can, owner, onEdit, onMeasure, onChanged }: { rows: FabricReceipt[]; can:(k:string)=>boolean; owner:boolean; onEdit:(x:FabricReceipt)=>void; onMeasure:(x:FabricReceipt)=>void; onChanged:()=>Promise<void> }) {
  const [quick,setQuick]=useState<FabricReceipt|null>(null);
  const [busyId,setBusyId]=useState<string|null>(null);
  const [monthFilter,setMonthFilter]=useState("");
  const monthOptions=useMemo(()=>Array.from(new Set(rows.map(r=>{
    const raw=r.receivedAt||r.createdAt||r.updatedAt;
    const d=raw?new Date(raw):null;
    return d&&!Number.isNaN(d.getTime())?`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`:"";
  }).filter(Boolean))).sort((a,b)=>b.localeCompare(a)),[rows]);
  ...
  const totals = useMemo(()=>({
    m: displayRows.reduce((s,x)=>s+receiptRollTotal(x,"supplierDeclaredM"),0),
    kg: displayRows.reduce((s,x)=>s+receiptRollTotal(x,"actualKg"),0),
    mDiff: displayRows.reduce((s,x)=>s+(receiptRollTotal(x,"actualM")-receiptRollTotal(x,"supplierDeclaredM")),0),
    kgDiff: displayRows.reduce((s,x)=>s+(receiptRollTotal(x,"actualKg")-receiptRollTotal(x,"supplierDeclaredKg")),0)
  }),[displayRows]);
  ...
  return <>
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="text-sm font-semibold">Phiếu nhập vải</div>
      <select className="rounded-xl border bg-white px-3 py-2 text-xs font-semibold" value={monthFilter} onChange={e=>setMonthFilter(e.target.value)}>
        <option value="">Tất cả tháng</option>
        {monthOptions.map(m=>{const [y,mm]=m.split("-");return <option key={m} value={m}>Tháng {mm}/{y}</option>})}
      </select>
    </div>
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Mét nhập kho (NCC)",`${fmt(totals.m,3)} m`],["Kg cân thực tế",`${fmt(totals.kg,3)} kg`],["Lệch mét kiểm tra",`${totals.mDiff>0?"+":""}${fmt(totals.mDiff,3)} m`],["Lệch kg",`${totals.kgDiff>0?"+":""}${fmt(totals.kgDiff,3)} kg`]].map(([l,v])=><Card key={l} className="p-4"><div className="text-xs font-semibold uppercase tracking-wide text-neutral-400">{l}</div><div className="mt-2 text-xl font-semibold">{v}</div></Card>)}</div>
    <Card className="overflow-hidden"><div className="overflow-x-auto"><table className="min-w-[1780px] w-full text-sm"><thead className="bg-neutral-50 text-left text-xs uppercase tracking-wide text-neutral-500"><tr><th className="px-3 py-2">Ngày nhập</th><th className="px-3 py-2">Phiếu / vải</th><th className="px-3 py-2">Mẫu</th><th className="px-3 py-2">NCC</th><th className="px-3 py-2">SL nhập kho (NCC)</th><th className="px-3 py-2">Cân / quy đổi kiểm tra</th><th className="px-3 py-2">Chênh lệch</th><th className="px-3 py-2">GSM</th><th className="px-3 py-2">Ghi chú</th><th className="px-3 py-2">Giá nhập</th><th className="px-3 py-2">Trạng thái</th><th className="px-3 py-2"></th></tr></thead><tbody className="divide-y divide-neutral-100">{displayRows.map(r=>{const declaredM=receiptRollTotal(r,"supplierDeclaredM"),declaredKg=receiptRollTotal(r,"supplierDeclaredKg"),actualM=receiptRollTotal(r,"actualM"),actualKg=receiptRollTotal(r,"actualKg");const dm=actualM-declaredM;const dkg=actualKg-declaredKg;const locked=r.status==="COMPLETED"||r.status==="CANCELLED";const receivedRaw=r.receivedAt||r.createdAt||r.updatedAt;const receivedDate=receivedRaw?new Date(receivedRaw):null;return <tr key={r.id} onClick={()=>setQuick(r)} className="cursor-pointer align-middle hover:bg-neutral-50/80"><td className="px-3 py-2 whitespace-nowrap"><b>{receivedDate&&!Number.isNaN(receivedDate.getTime())?receivedDate.toLocaleDateString("vi-VN"):"—"}</b><div className="text-[10px] text-neutral-400">{receivedDate&&!Number.isNaN(receivedDate.getTime())?`Tháng ${String(receivedDate.getMonth()+1).padStart(2,"0")}/${receivedDate.getFullYear()}`:""}</div></td><td className="px-3 py-2"><b className="text-[12px]">{r.receiptCode}</b><div className="text-[11px] leading-4 text-neutral-500">{r.fabricName || r.fabricCode || "Vải"} · {r.colorName || r.colorCode || "—"}</div><div className="text-[11px] leading-4 text-neutral-400">Lô {r.lotCode || "—"} · {r.rollCount} cây · bấm hàng để xem cây</div></td><td className="px-3 py-2">{r.designSample ? <><b>{r.designSample.code}</b><div className="text-[11px] leading-4 text-neutral-500">{r.designSample.name}</div></> : "—"}</td><td className="px-3 py-2">{can("fabric_receipt.supplier_identity.view") ? (r.supplier ? `${r.supplier.code || "NCC"} · ${r.supplier.name || ""}`.trim() : "—") : (r.supplier?.code || "—")}<div className="text-[11px] leading-4 text-neutral-400">{r.branch?.name || ""}</div></td><td className="px-3 py-2">{fmt(declaredM,3)} m<div className="text-[11px] leading-4 text-neutral-500">{fmt(declaredKg,3)} kg</div></td><td className="px-3 py-2 font-semibold">{fmt(actualM,3)} m<div className="text-[11px] font-normal leading-4 text-neutral-500">{fmt(actualKg,3)} kg</div></td><td className={`px-3 py-2 font-semibold ${dm<0||dkg<0?"text-red-700":"text-emerald-700"}`}>{dm>0?"+":""}{fmt(dm,3)} m<div className="text-xs">{dkg>0?"+":""}{fmt(dkg,3)} kg</div>{r.varianceApproved && <div className="mt-1 text-[11px] text-blue-700">Đã duyệt lệch</div>}</td><td className="px-3 py-2">{r.measuredGsm ? <b>{fmt(r.measuredGsm,1)} GSM</b> : "—"}<div className="text-[11px] leading-4 text-neutral-400">NCC: {r.expectedGsm ? `${fmt(r.expectedGsm,1)}` : "—"}</div></td><td className="max-w-[220px] px-3 py-2"><div className="line-clamp-3 text-xs text-neutral-600">{r.note||"—"}</div></td><td className="px-3 py-2">{owner?(()=>{const c=receiptCostSummaryForDisplay(r);return <>{c.grandTotalVnd>0?<b>{money(c.grandTotalVnd)}</b>:"Chưa có giá"}<div className="text-[11px] leading-4 text-neutral-400">Tiền vải {money(c.goodsVnd)}</div><div className="text-[11px] leading-4 text-neutral-400">Phí {money(c.totalShippingVnd)}</div></>})():"—"}</td><td className="px-3 py-2"><Badge status={r.status}>{statusLabel(r.status,RECEIPT_STATUSES)}</Badge></td><td className="px-3 py-2" onClick={e=>e.stopPropagation()}><div className="flex min-w-[430px] flex-wrap items-center justify-end gap-1"><button type="button" onClick={()=>printFabricReceipt(r,can("fabric_receipt.supplier_identity.view"))} className="rounded-lg border border-neutral-300 bg-white px-2.5 py-1 text-[11px] font-semibold whitespace-nowrap">In phiếu nhận</button>{can("fabric_receipt.edit") && !locked && <button onClick={()=>onEdit(r)} className="rounded-lg border border-neutral-300 px-2.5 py-1 text-[11px] font-semibold whitespace-nowrap">Sửa phiếu</button>}{can("fabric_receipt.measure") && !locked && <button onClick={()=>onMeasure(r)} className="rounded-lg border border-neutral-300 px-2.5 py-1 text-[11px] font-semibold whitespace-nowrap">Cân mẫu GSM</button>}{can("fabric_receipt.approve_variance") && !locked && !r.varianceApproved && (Math.abs(dm)>0.001 || Math.abs(dkg)>0.001) && <button onClick={async()=>{await api(`/sample-fabric/fabric-receipts/${r.id}/approve-variance`,{method:"POST",body:"{}"});await onChanged()}} className="rounded-lg border border-amber-300 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-800 whitespace-nowrap">Duyệt chênh lệch</button>}{can("fabric_receipt.complete") && !locked && <button onClick={async()=>{await api(`/sample-fabric/fabric-receipts/${r.id}/complete`,{method:"POST",body:"{}"});await onChanged()}} className="rounded-lg bg-neutral-950 px-2.5 py-1 text-[11px] font-semibold text-white whitespace-nowrap">Hoàn tất</button>}{can("fabric_receipt.cancel") && !locked && <button disabled={busyId===r.id} onClick={()=>cancelReceipt(r)} className="rounded-lg border border-red-300 bg-red-50 px-2.5 py-1 text-[11px] font-semibold text-red-700 disabled:opacity-50 whitespace-nowrap">Huỷ phiếu</button>}{can("fabric_receipt.delete") && r.status!=="COMPLETED" && <button disabled={busyId===r.id} onClick={()=>deleteReceipt(r)} className="rounded-lg bg-red-600 px-2.5 py-1 text-[11px] font-semibold text-white disabled:opacity-50 whitespace-nowrap">Xoá phiếu</button>}</div></td></tr>})}</tbody></table></div></Card>
    {!displayRows.length && <Card className="p-12 text-center text-sm text-neutral-500">Chưa có phiếu vải phù hợp.</Card>}
    {quick&&<FabricReceiptQuickView receipt={quick} can={can} canSeeCost={owner} onClose={()=>setQuick(null)}/>}
  </>;
}
...
function SampleForm({ sample, initialLane, measurementTemplates, boards, staff, seasons, productGroups, factories, samplePeople, canUpload, canViewFabricLink, onPeopleChanged, onClose, onSaved }: { sample: Sample | null; initialLane:"IDEA"|"DEPLOY"|"FABRIC_SAMPLE"; measurementTemplates:MeasurementTemplate[]; boards: FabricBoard[]; staff: Staff[]; seasons: string[]; productGroups: string[]; factories: Factory[]; samplePeople:SamplePerson[]; canUpload: boolean; canViewFabricLink: boolean; onPeopleChanged:(rows:SamplePerson[])=>void; onClose: () => void; onSaved: () => void }) {
  const lane=String(sample?.priorityLane||initialLane||"IDEA") as "IDEA"|"DEPLOY"|"FABRIC_SAMPLE";
  const [form,setForm]=useState<any>({name:sample?.name||"",code:sample?.code||"",year:sample?.year||new Date().getFullYear(),season:sample?.season||"",category:sample?.category||"",fabricBoardId:sample?.fabricBoardId||"",fabricColorId:sample?.fabricColorId||"",fabricColorName:sample?.fabricColorName||sample?.fabricColor?.name||"",fabricColorCode:sample?.fabricColorCode||sample?.fabricColor?.code||"",sampleFactoryId:sample?.sampleFactoryId||"",sampleFactoryName:sample?.sampleFactoryName||"",sampleMakerId:sample?.sampleMakerId||"",sampleMakerName:sample?.sampleMakerName||"",patternMakerId:sample?.patternMakerId||"",patternMakerName:sample?.patternMakerName||"",status:sample?.status||(initialLane==="DEPLOY"?"FABRIC_SELECTED":"IDEA"),fabricSampleReceivedAt:sample?.fabricSampleReceivedAt?sample.fabricSampleReceivedAt.slice(0,10):"",assigneeStaffId:sample?.assigneeStaffId||"",assigneeName:sample?.assigneeName||"",nextAction:sample?.nextAction||"",dueDate:sample?.dueDate?sample.dueDate.slice(0,10):"",note:sample?.note||"",technicalNote:sample?.technicalNote||"",coverImageUrl:sample?.coverImageUrl||""});
  ...
  const [sampleImages,setSampleImages]=useState<Array<{type?:string;url:string;caption?:string|null}>>(()=>{
    const rows=(Array.isArray(sample?.images)?sample!.images!.filter((x:any)=>!isPatternAsset(x)).map((x:any)=>({type:x.type||"SAMPLE",url:String(x.url),caption:x.caption||"Ảnh mẫu / ảnh tham khảo"})):[]);
    if(sample?.coverImageUrl&&!rows.some(x=>x.url===sample.coverImageUrl))rows.unshift({type:"SAMPLE",url:sample.coverImageUrl,caption:"Ảnh mẫu / ảnh tham khảo"});
    return rows;
  });
  const [fabricSampleColors,setFabricSampleColors]=useState<Array<{id?:string;colorName:string;colorCode:string;receivedMeters:string}>>(()=>lane==="FABRIC_SAMPLE"&&sample?.fabricSampleColors?.length
    ? sample.fabricSampleColors.map((x:any)=>({id:x.id,colorName:x.colorName||"",colorCode:x.colorCode||"",receivedMeters:String(x.receivedMeters??"")}))
    : [{colorName:"",colorCode:"",receivedMeters:""}]
  );
  ...
  useEffect(()=>{
    if(lane==="FABRIC_SAMPLE")return;
    api<FabricSampleOption[]>("/sample-fabric/samples/fabric-sample-options")
      .then(setFabricSampleOptions)
      .catch(()=>setFabricSampleOptions([]));
  },[lane,sample?.id]);
  ...
  const filteredFabricBoards=useMemo(()=>{
    ...
    return boards.filter(b=>[b.boardCode,b.fabricCode,b.name,b.supplier?.code,b.supplier?.name,b.composition]
      .some(v=>String(v||"").toLocaleLowerCase("vi-VN").includes(key)));
  },[boards,fabricBoardSearch]);
  ...
  async function uploadSampleImages(files:FileList|File[]){
    try{
      setError("");
      ...
      for(const file of picked){
        const r=await uploadWorkspaceFile("/sample-fabric/samples/upload",file);
        uploaded.push({type:"SAMPLE",url:r.url,caption:"Ảnh mẫu / ảnh tham khảo"});
      }
      setSampleImages(current=>{
        const next=[...current];
        for(const image of uploaded)if(!next.some(x=>x.url===image.url))next.push(image);
        return next;
      });
      if(!form.coverImageUrl&&uploaded[0]?.url)patch("coverImageUrl",uploaded[0].url);
    }catch(e){setError(e instanceof Error?e.message:"Không tải được ảnh mẫu.")}
  }
  function removeSampleImage(url:string){
    setSampleImages(current=>{
      const next=current.filter(x=>x.url!==url);
      if(form.coverImageUrl===url)patch("coverImageUrl",next[0]?.url||"");
      return next;
    });
  }
  ...
  async function save(){
    try{
      ...
      if(code&&codeCheck.available!==true)throw new Error(codeCheck.message||"Mã mẫu chưa được xác nhận là hợp lệ.");
      const assigned=staff.find(x=>x.id===form.assigneeStaffId);
      const factory=factories.find(x=>x.id===form.sampleFactoryId);
      const sampleMaker=people.find(x=>x.id===form.sampleMakerId);
      const patternMaker=people.find(x=>x.id===form.patternMakerId);

      if(lane==="FABRIC_SAMPLE"){
        const validColors=fabricSampleColors.filter(x=>x.colorName.trim()&&num(x.receivedMeters)>0);
        if(!validColors.length)throw new Error("Vải mẫu phải có ít nhất 1 màu và số mét nhận lớn hơn 0.");
      }

      const saved=await api<Sample>(sample?`/sample-fabric/samples/${sample.id}`:"/sample-fabric/samples",{
        method:sample?"PATCH":"POST",
        body:JSON.stringify({
          name:form.name,code,year:form.year,season:form.season,category:titleCaseVi(form.category),
          fabricBoardId:form.fabricBoardId||null,fabricColorId:null,
          fabricColorName:form.fabricColorName||null,fabricColorCode:normalizeColorCode(form.fabricColorCode)||null,
          sampleFactoryId:form.sampleFactoryId||null,sampleFactoryName:factory?.name||form.sampleFactoryName||null,
          sampleMakerId:form.sampleMakerId||null,sampleMakerName:sampleMaker?.name||form.sampleMakerName||null,
          patternMakerId:form.patternMakerId||null,patternMakerName:patternMaker?.name||form.patternMakerName||null,
          status:form.status,priorityLane:sample?.priorityLane||initialLane,
          fabricSampleReceivedAt:lane==="FABRIC_SAMPLE"?(form.fabricSampleReceivedAt||null):null,
          ...(lane==="FABRIC_SAMPLE"?{
            fabricSampleColors:fabricSampleColors
              .filter(x=>x.colorName.trim()&&num(x.receivedMeters)>0)
              .map((x,index)=>({
                id:x.id||undefined,
                colorName:titleCaseVi(x.colorName),
                colorCode:normalizeColorCode(x.colorCode)||null,
                receivedMeters:num(x.receivedMeters),
                sortOrder:index,
              }))
          }:{}),
          assigneeStaffId:form.assigneeStaffId||null,assigneeName:assigned?.name||form.assigneeName||null,
          nextAction:form.nextAction||null,dueDate:form.dueDate||null,note:form.note||null,technicalNote:form.technicalNote||null,
          coverImageUrl:form.coverImageUrl||sampleImages[0]?.url||null,
          images:[
            ...sampleImages.map(x=>({type:x.type||"SAMPLE",url:x.url,caption:x.caption||"Ảnh mẫu / ảnh tham khảo"})),
      ...
      if(lane!=="FABRIC_SAMPLE"){
        if(selectedFabricColorId&&num(selectedFabricMeters)<=0)throw new Error("Nhập số mét vải dùng cho mẫu.");
        await api(`/sample-fabric/samples/${saved.id}/fabric-sample`,{
          method:"PATCH",
          body:JSON.stringify(selectedFabricColorId
            ? {fabricSampleColorId:selectedFabricColorId,meters:num(selectedFabricMeters)}
            : {fabricSampleColorId:null}
          )
        });
      }

      saveMeasurementSnapshot(code,measurement);
      onSaved();
    }catch(e){
  ...
  return <Modal title={sample?`${lane==="FABRIC_SAMPLE"?"Sửa vải mẫu":"Sửa mẫu"} ${sample.code}`:lane==="FABRIC_SAMPLE"?"Tạo vải mẫu":"Tạo mẫu triển khai"} onClose={onClose} wide><div className="space-y-5 p-5">{error&&<div className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}<div className="rounded-2xl border border-blue-100 bg-blue-50/60 px-4 py-3 text-sm text-blue-900">{lane==="FABRIC_SAMPLE"?<>Quản lý <b>vải mẫu</b>: ngày nhận, ảnh, thông tin, người phụ trách và ghi chú. Dữ liệu này tách riêng, không tính vào Ý tưởng hay Triển khai mẫu.</>:<>Trang này chỉ quản lý <b>mẫu gửi đi / tiến độ mẫu</b>. Thành phần, GSM, NCC, mã chất vải và danh sách màu được quản lý tại <Link href="/fabric-library" className="font-semibold underline">Bảng vải</Link>.</>}</div>{lane==="FABRIC_SAMPLE"&&<div className="space-y-3 rounded-2xl border border-orange-200 bg-orange-50/40 p-4">
  <div className="flex items-center justify-between gap-3">
    <div>
      <b>Các màu vải mẫu</b>
      <div className="mt-0.5 text-xs text-neutral-500">Mỗi màu có mã màu và số mét nhận riêng. Một màu có thể dùng cho nhiều mẫu cho tới khi hết mét.</div>
    </div>
    <button type="button" onClick={()=>setFabricSampleColors(rows=>[...rows,{colorName:"",colorCode:"",receivedMeters:""}])} className="shrink-0 rounded-xl bg-orange-600 px-3 py-2 text-xs font-semibold text-white">+ Thêm màu</button>
  </div>
  <div className="space-y-2">
    {fabricSampleColors.map((row,i)=><div key={row.id||i} className="grid gap-2 rounded-2xl border bg-white p-3 md:grid-cols-[1fr_150px_150px_auto]">
      <input className={inputClass} value={row.colorName} onChange={e=>setFabricSampleColors(rows=>rows.map((x,j)=>j===i?{...x,colorName:e.target.value}:x))} placeholder="Tên màu, VD: Xanh"/>
      <input className={inputClass} value={row.colorCode} onChange={e=>setFabricSampleColors(rows=>rows.map((x,j)=>j===i?{...x,colorCode:e.target.value}:x))} placeholder="#12"/>
      <div className="relative">
        <input inputMode="decimal" className={`${inputClass} pr-9`} value={row.receivedMeters} onChange={e=>setFabricSampleColors(rows=>rows.map((x,j)=>j===i?{...x,receivedMeters:decimalRaw(e.target.value)}:x))} placeholder="2"/>
        <span className="pointer-events-none absolute right-3 top-2.5 text-sm text-neutral-400">m</span>
      </div>
      <button type="button" disabled={fabricSampleColors.length<=1} onClick={()=>setFabricSampleColors(rows=>rows.filter((_,j)=>j!==i))} className="rounded-xl border border-red-200 px-3 text-xs font-semibold text-red-600 disabled:opacity-30">Xoá</button>
    </div>)}
  </div>
</div>}

{lane!=="FABRIC_SAMPLE"&&<div className="space-y-3 rounded-2xl border border-orange-200 bg-orange-50/40 p-4">
  <div>
    <b>Vải may mẫu</b>
    <div className="mt-0.5 text-xs text-neutral-500">Chọn vải mẫu, đúng màu và số mét sử dụng. Liên kết này giữ nguyên khi Ý tưởng chuyển sang Triển khai.</div>
  </div>
  <div className="grid gap-3 md:grid-cols-[1.5fr_.6fr]">
    <Field label="Vải mẫu / màu">
      <select className={inputClass} value={selectedFabricColorId} onChange={e=>{setSelectedFabricColorId(e.target.value);if(!e.target.value)setSelectedFabricMeters("")}}>
        <option value="">Chưa chọn vải mẫu</option>
        {fabricSampleOptions.flatMap(f=>f.fabricSampleColors
          .filter(c=>c.availableMeters>0||c.id===selectedFabricColorId)
          .map(c=><option key={c.id} value={c.id}>{f.name} · {c.colorName}{c.colorCode?` ${c.colorCode}`:""} · còn {fmt(c.availableMeters,3)}m</option>)
        )}
      </select>
    </Field>
    <Field label="Số mét dùng">
      <div className="relative">
        <input inputMode="decimal" disabled={!selectedFabricColorId} className={`${inputClass} pr-9 disabled:bg-neutral-100`} value={selectedFabricMeters} onChange={e=>setSelectedFabricMeters(decimalRaw(e.target.value))} placeholder="VD: 1"/>
        <span className="pointer-events-none absolute right-3 top-2.5 text-sm text-neutral-400">m</span>
      </div>
    </Field>
  </div>
  {currentFabricAllocation&&<div className="rounded-xl border bg-white px-3 py-2 text-xs text-neutral-600">
    Đang dùng: <b>{currentFabricAllocation.fabricSampleColor?.fabricSample?.name||"Vải mẫu"} · {currentFabricAllocation.fabricSampleColor?.colorName}{currentFabricAllocation.fabricSampleColor?.colorCode?` ${currentFabricAllocation.fabricSampleColor.colorCode}`:""} · {fmt(currentFabricAllocation.meters,3)}m</b>
  </div>}
</div>}

<div className="grid gap-4 md:grid-cols-3">
    <Field label="Tên mẫu"><input className={inputClass} value={form.name} onChange={e=>patch("name",e.target.value)}/></Field><Field label="Mã mẫu"><div><input className={`${inputClass} ${codeCheck.available===false?"border-red-400":codeCheck.available===true?"border-emerald-400":""}`} value={form.code} onChange={e=>patch("code",normalizeSampleCode(e.target.value))} placeholder="VD: QSK925"/>{form.code&&<p className={`mt-1.5 text-xs ${codeCheck.loading?"text-neutral-400":codeCheck.available?"text-emerald-600":"text-red-600"}`}>{codeCheck.loading?"Đang kiểm tra mã...":codeCheck.message}</p>}</div></Field><Field label="Năm"><input type="number" className={inputClass} value={form.year} onChange={e=>patch("year",e.target.value)}/></Field>{lane==="FABRIC_SAMPLE"&&<Field label="Ngày nhận vải mẫu"><input type="date" className={inputClass} value={form.fabricSampleReceivedAt} onChange={e=>patch("fabricSampleReceivedAt",e.target.value)}/></Field>}
    <Field label="Mùa / BST"><select className={inputClass} value={form.season} onChange={e=>patch("season",e.target.value)}><option value="">Chưa chọn</option>{seasons.map(x=><option key={x} value={x}>{x}</option>)}</select></Field><Field label="Nhóm sản phẩm"><div className="space-y-2"><select className={inputClass} value={customCategory?"__NEW__":form.category} onChange={e=>{if(e.target.value==="__NEW__"){setCustomCategory(true);patch("category","")}else{setCustomCategory(false);patch("category",e.target.value)}}}><option value="">Chưa chọn</option>{uniqueTextValues([...productGroups,!customCategory?form.category:""]).map(x=><option key={x} value={x}>{x}</option>)}<option value="__NEW__">+ Thêm nhóm mới</option></select>{customCategory&&<input className={inputClass} value={form.category} onChange={e=>patch("category",e.target.value)} onBlur={()=>patch("category",titleCaseVi(form.category))} placeholder="VD: Áo Khoác"/>}</div></Field>{canViewFabricLink && (<Field label="Bảng vải"><div className="space-y-2">
  <input className={inputClass} value={fabricBoardSearch} onChange={e=>setFabricBoardSearch(e.target.value)} placeholder="Tìm mã bảng, mã vải, tên vải, NCC..."/>
  <select className={inputClass} value={form.fabricBoardId} onChange={e=>{patch("fabricBoardId",e.target.value);patch("fabricColorId","");setFabricBoardSearch("")}}>
    <option value="">Chưa chọn bảng vải</option>
    {selectedBoard&&!filteredFabricBoards.some(b=>b.id===selectedBoard.id)&&<option value={selectedBoard.id}>{selectedBoard.boardCode}{selectedBoard.name?` · ${selectedBoard.name}`:""}</option>}
    {filteredFabricBoards.map(b=><option key={b.id} value={b.id}>{b.boardCode}{b.fabricCode?` · ${b.fabricCode}`:""}{b.name?` · ${b.name}`:""}</option>)}
  </select>
  {fabricBoardSearch&&<div className="text-[11px] text-neutral-400">Tìm thấy {filteredFabricBoards.length} bảng vải.</div>}
</div></Field>)}
...
function ReceiptForm({ receipt, suppliers, branches, staff, samples, products, boards, canFabricBoardLink, canSupplierIdentity, canCostView, canCostEdit, canUpload, onClose, onSaved }: { receipt:FabricReceipt|null; suppliers:Supplier[]; branches:Branch[]; staff:Staff[]; samples:any[]; products:Array<{id:string;name:string;slug:string;imageUrl?:string|null}>; boards:FabricBoard[]; canFabricBoardLink:boolean; canSupplierIdentity:boolean; canCostView:boolean; canCostEdit:boolean; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
  ...
  const legacyReceiptUnitPriceCny=receipt?receiptLegacyPriceCny(receipt):0;
  const [rolls,setRolls]=useState<Roll[]>(receipt?.rolls?.length?receipt.rolls.map((r,i)=>({...r,sortOrder:r.sortOrder??i+1,fabricCode:r.fabricCode||receipt.fabricCode||"",unitPriceCny:num(r.unitPriceCny)>0?r.unitPriceCny:(legacyReceiptUnitPriceCny>0?legacyReceiptUnitPriceCny:""),priceUnit:r.priceUnit||receipt.priceUnit||"METER"})):[{sortOrder:1,fabricCode:receipt?.fabricCode||"",rollCode:"",colorName:"",colorCode:"",supplierDeclaredM:"",supplierDeclaredKg:"",actualM:"",actualKg:"",measuredGsm:"",unitPriceCny:"",priceUnit:"METER",defectNote:"",passed:true}]);
  ...
  const receiptFabricCodes=useMemo(()=>{
    const values=[...fabricConfigs.map(x=>x.fabricCode),...colorMaps.map(x=>x.fabricCode),...rolls.map(x=>x.fabricCode)].map(x=>String(x||"").replace(/\s+/g," ").trim()).filter(Boolean);
    const seen=new Set<string>();
    return values.filter(value=>{const key=value.toLocaleUpperCase("vi-VN");if(seen.has(key))return false;seen.add(key);return true});
  ...
  function patchFabricConfig(i:number,key:keyof FabricConfig,value:any){
    setFabricConfigs(rows=>rows.map((row,j)=>{
      ...
      const oldCode=String(row.fabricCode||"").replace(/\s+/g," ").trim().toLocaleUpperCase("vi-VN");
      const newCode=String(value||"").replace(/\s+/g," ");
      const newKey=newCode.trim().toLocaleUpperCase("vi-VN");
      if(oldCode&&newKey&&oldCode!==newKey){
        setColorMaps(cm=>cm.map(x=>String(x.fabricCode||"").replace(/\s+/g," ").trim().toLocaleUpperCase("vi-VN")===oldCode?{...x,fabricCode:newCode.trim()}:x));
        setRolls(rr=>rr.map(x=>String(x.fabricCode||"").replace(/\s+/g," ").trim().toLocaleUpperCase("vi-VN")===oldCode?{...x,fabricCode:newCode.trim()}:x));
        setFabricCosts(cc=>cc.map(x=>String(x.fabricCode||"").replace(/\s+/g," ").trim().toLocaleUpperCase("vi-VN")===oldCode?{...x,fabricCode:newCode.trim()}:x));
      }
  ...
  const fabricCodes=(()=>{const values=[...receiptFabricCodes,...rolls.map(r=>String(r.fabricCode||"").replace(/\s+/g," ").trim()).filter(Boolean)];const seen=new Set<string>();return values.filter(value=>{const key=value.toLocaleUpperCase("vi-VN");if(seen.has(key))return false;seen.add(key);return true})})();
  ...
  return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose} wide><div className="space-y-5 p-5">{error&&<div className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
    <div className="grid gap-4 md:grid-cols-4">
      ...
      <Field label="Ngày nhập vải"><input type="date" className={inputClass} value={form.receivedAt} onChange={e=>patch("receivedAt",e.target.value)}/></Field>
    ...
    {canCostView&&receipt&&(num(receipt.unitPrice)>0||num(receipt.unitPriceVnd)>0)&&<div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950"><b>Giá lịch sử của phiếu:</b> {fmt(num(receipt.unitPrice)>0?receipt.unitPrice:receipt.unitPriceVnd,2)} {num(receipt.unitPrice)>0?String(receipt.priceCurrency||"VND").toUpperCase():"VND"}/{receipt.priceUnit==="KG"?"kg":receipt.priceUnit==="ROLL"?"cây":"m"}{num(receipt.exchangeRateToVnd)>0?` · Tỷ giá ${fmt(receipt.exchangeRateToVnd,0)} VND/CNY`:""}. Giá này được tự đưa xuống từng cây chưa có giá để giữ dữ liệu phiếu cũ.</div>}
    {canCostView&&<div className="rounded-3xl bg-amber-50 p-4"><div className="grid gap-4 md:grid-cols-3"><Field label="Tỷ giá lúc nhập"><div className="relative"><input disabled={!canCostEdit} inputMode="decimal" className={`${inputClass} pr-12 disabled:bg-neutral-100`} value={moneyInput(form.exchangeRateToVnd)} onChange={e=>patch("exchangeRateToVnd",moneyRaw(e.target.value))} placeholder="VD: 3.920"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400">VND/CNY</span></div></Field><div className="rounded-2xl bg-white p-3 text-sm"><div className="text-[10px] font-semibold uppercase text-neutral-400">Tiền vải</div><div className="mt-1 text-lg font-semibold">{liveCostSummary.goodsCny>0?`${fmt(liveCostSummary.goodsCny,2)} CNY`:(liveCostSummary.goodsVnd>0?money(liveCostSummary.goodsVnd):"Chưa có dữ liệu giá")}</div>{liveCostSummary.goodsCny>0&&<div className="text-xs text-neutral-500">{money(liveCostSummary.goodsVnd)}</div>}</div><div className="rounded-2xl bg-white p-3 text-sm"><div className="text-[10px] font-semibold uppercase text-neutral-400">Tổng đơn</div><div className="mt-1 text-lg font-semibold">{liveCostSummary.grandTotalVnd>0?money(liveCostSummary.grandTotalVnd):"Chưa có dữ liệu"}</div><div className="text-xs text-neutral-500">Đã gồm vận chuyển</div></div></div></div>}
    ...
    <div className="rounded-3xl border p-4">
      ...
      <div className="mt-4 space-y-3">{fabricConfigs.map((cfg,ci)=>{const code=String(cfg.fabricCode||"").trim().toUpperCase();const source=configSource[ci]||(cfg.productId?"PRODUCT":"DESIGN");const colors=colorMaps.map((x,index)=>({x,index})).filter(({x})=>String(x.fabricCode||"").trim().toUpperCase()===code);return <div key={cfg.id||ci} className="rounded-2xl bg-neutral-50 p-3">
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-5">
          <Field label="Tên Mã Vải"><input className={inputClass} value={cfg.fabricCode||""} onChange={e=>patchFabricConfig(ci,"fabricCode",e.target.value)} onBlur={e=>patchFabricConfig(ci,"fabricCode",e.target.value.replace(/\s+/g," ").trim())} placeholder="VD: Bông tấm không trần"/></Field>
  ...
  <table className="min-w-[1500px] w-full border-collapse text-xs">
    <thead className="bg-neutral-100">
      <tr>{["STT","Mã vải","Mã cây","Màu đã cấu hình","Mã màu","NCC m nhập kho","NCC kg","Kg cân thực tế","GSM","Khổ cm","M quy đổi kiểm tra","Quy đổi","Giá CNY","ĐVT giá","Ghi chú",""].map(x=><th key={x} className="border-b px-2 py-2 text-left font-semibold">{x}</th>)}</tr>
    </thead>
    <tbody>{rolls.map((r,i)=>{
