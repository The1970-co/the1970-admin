...
type Roll = { id?: string; rollCode?: string | null; images?: Array<{id:string;url:string;caption?:string|null}>; imageUrl?: string | null; colorName?: string | null; colorCode?: string | null; supplierDeclaredM?: number | string | null; supplierDeclaredKg?: number | string | null; actualM?: number | string | null; actualKg?: number | string | null; defectNote?: string | null; passed?: boolean };
...
export default function SampleFabricWorkspaceClient({ defaultSection }: { defaultSection: Section }) {
  ...
  return <div className="space-y-5">
    ...
    {showReceiptForm && <ReceiptForm receipt={editingReceipt} suppliers={suppliers} branches={branches} samples={metaSamples} boards={workspaceMeta.boards || []} canSupplierIdentity={can("fabric_receipt.supplier_identity.view")} canCostView={can("fabric_receipt.cost.view") || can("fabric_receipt.cost.edit")} canCostEdit={can("fabric_receipt.cost.edit")} canUpload={can("fabric_receipt.upload_images")} onClose={()=>setShowReceiptForm(false)} onSaved={async()=>{setShowReceiptForm(false);await reload()}} />}
...
function ReceiptForm({ receipt, suppliers, branches, samples, boards, canSupplierIdentity, canCostView, canCostEdit, canUpload, onClose, onSaved }: { receipt:FabricReceipt|null; suppliers:Supplier[]; branches:Branch[]; samples:any[]; boards:FabricBoard[]; canSupplierIdentity:boolean; canCostView:boolean; canCostEdit:boolean; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
  ...
  const [rolls,setRolls]=useState<Roll[]>(receipt?.rolls?.length?receipt.rolls:[{rollCode:"",colorName:"",colorCode:"",supplierDeclaredM:"",supplierDeclaredKg:"",actualM:"",actualKg:"",defectNote:"",passed:true}]);
  ...
  async function save(){try{setSaving(true);setError("");const normalizedRolls=rolls.map(r=>({...r,colorCode:normalizeColorCode(String(r.colorCode||""))||null,defectNote:String(r.defectNote||"").trim()||null}));const saved=await api<FabricReceipt>(receipt?`/sample-fabric/fabric-receipts/${receipt.id}`:"/sample-fabric/fabric-receipts",{method:receipt?"PATCH":"POST",body:JSON.stringify({...form,colorCode:normalizeColorCodes(form.colorCode)||null,unitPrice:undefined,priceUnit:undefined,priceCurrency:undefined,exchangeRateToVnd:undefined,rollCount:normalizedRolls.length,rolls:normalizedRolls})});if(canCostEdit && form.unitPrice!=="") await api(`/sample-fabric/fabric-receipts/${saved.id}/cost`,{method:"PATCH",body:JSON.stringify({unitPrice:form.unitPrice,priceUnit:form.priceUnit,priceCurrency:form.priceCurrency||"VND",exchangeRateToVnd:form.priceCurrency==="CNY"?form.exchangeRateToVnd:1})});for(const [indexText,files] of Object.entries(rollFiles)){const index=Number(indexText);const serverRoll=saved.rolls?.[index];if(!serverRoll?.id)continue;for(const file of files){const uploaded=await uploadWorkspaceFile("/sample-fabric/fabric-receipts/upload",file);await api(`/sample-fabric/fabric-receipts/${saved.id}/images`,{method:"POST",body:JSON.stringify({rollId:serverRoll.id,type:"FABRIC",url:uploaded.url,caption:`Ảnh ${serverRoll.rollCode||`cây ${index+1}`}`})})}}onSaved();}catch(e){setError(e instanceof Error?e.message:"Không lưu được phiếu.")}finally{setSaving(false)}}
  ...
  return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose} wide><div className="space-y-5 p-5">{error&&<div className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Field label="Bảng vải"><select className={inputClass} value={form.fabricBoardId||selectedBoard?.id||""} onChange={e=>chooseBoard(e.target.value)}><option value="">Chưa liên kết bảng vải</option>{boards.map(b=><option key={b.id} value={b.id}>{b.boardCode}{b.fabricCode?` · ${b.fabricCode}`:""}{b.name?` · ${b.name}`:""}</option>)}</select></Field>
    ...
    {canCostView&&<div className="rounded-3xl bg-amber-50 p-4"><div className="grid gap-4 md:grid-cols-4"><Field label="Đơn giá"><input type="number" className={inputClass} value={form.unitPrice} onChange={e=>patch("unitPrice",e.target.value)}/></Field><Field label="Tiền tệ"><select className={inputClass} value={form.priceCurrency} onChange={e=>patch("priceCurrency",e.target.value)}><option value="VND">VND · Việt Nam</option><option value="CNY">CNY · Nhân dân tệ</option></select></Field><Field label="Tính giá theo"><select className={inputClass} value={form.priceUnit} onChange={e=>patch("priceUnit",e.target.value)}><option value="METER">Mét</option><option value="KG">Kg</option><option value="ROLL">Cây</option></select></Field>{form.priceCurrency==="CNY"&&<Field label="Tỷ giá 1 CNY"><div className="relative"><input inputMode="decimal" className={`${inputClass} pr-12`} value={form.exchangeRateToVnd} onChange={e=>patch("exchangeRateToVnd",e.target.value)} placeholder="VD: 3650"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-neutral-400">VND</span></div></Field>}</div>{convertedVnd>0&&<div className="mt-3 rounded-2xl bg-white p-3 text-sm"><span className="text-neutral-500">Đơn giá quy đổi: </span><b>{money(convertedVnd)}</b> / {form.priceUnit==="KG"?"kg":form.priceUnit==="ROLL"?"cây":"m"}</div>}</div>}

    <div><div className="mb-2 flex items-center justify-between"><div><b className="text-sm">Chi tiết từng cây vải</b><div className="text-xs text-neutral-400">Mỗi cây chọn một trong các màu phía trên; chưa khai màu thì nhập tay.</div></div><button onClick={()=>setRolls(x=>[...x,{rollCode:"",colorName:allowedColors.length===1?allowedColors[0].name:"",colorCode:allowedColors.length===1?allowedColors[0].code:"",supplierDeclaredM:"",supplierDeclaredKg:"",actualM:"",actualKg:"",defectNote:"",passed:true}])} className="rounded-xl border px-3 py-1.5 text-xs font-semibold">+ Thêm cây</button></div><div className="space-y-3">{rolls.map((r,i)=><div key={i} className="rounded-2xl bg-neutral-50 p-3"><div className="grid gap-2 md:grid-cols-4"><input className={inputClass} placeholder={`Mã cây ${i+1}`} value={r.rollCode||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,rollCode:e.target.value}:v))}/>{allowedColors.length>0&&!manualRollColor[i]?<><select className={inputClass} value={r.colorCode||""} onChange={e=>{const c=allowedColors.find(x=>x.code===e.target.value);setRolls(x=>x.map((v,j)=>j===i?{...v,colorCode:c?.code||"",colorName:c?.name||""}:v))}}><option value="">Chọn màu cây</option>{allowedColors.map(c=><option key={c.code} value={c.code}>{c.name?`${c.name} · `:""}{c.code}</option>)}</select><button type="button" className="rounded-2xl border bg-white px-3 text-xs font-semibold" onClick={()=>setManualRollColor(x=>({...x,[i]:true}))}>Nhập màu tay</button></>:<><input className={inputClass} placeholder="Màu" value={r.colorName||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,colorName:e.target.value}:v))}/><input className={inputClass} placeholder="# Mã màu" value={r.colorCode||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,colorCode:e.target.value}:v))} onBlur={()=>setRolls(x=>x.map((v,j)=>j===i?{...v,colorCode:normalizeColorCode(String(v.colorCode||""))}:v))}/></>}</div><div className="mt-2 grid gap-2 md:grid-cols-4">{[["supplierDeclaredM","NCC m"],["supplierDeclaredKg","NCC kg"],["actualM","Thực m"],["actualKg","Thực kg"]].map(([k,p])=><input key={k} type="number" step="0.001" className={inputClass} placeholder={p} value={(r as any)[k]??""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,[k]:e.target.value}:v))}/>)}</div><div className="mt-2 grid gap-2 md:grid-cols-[1fr_auto]"><textarea className={`${inputClass} min-h-20`} placeholder="Ghi chú cây: lệch màu, đầu cây bẩn, thiếu mét..." value={r.defectNote||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,defectNote:e.target.value}:v))}/><button onClick={()=>{setRolls(x=>x.filter((_,j)=>j!==i));setRollFiles(current=>{const next={...current};delete next[i];return next})}} className="rounded-xl px-3 text-sm text-red-600">Xoá cây</button></div>{canUpload&&<div className="mt-2 flex flex-wrap items-center gap-2"><label className="cursor-pointer rounded-xl border bg-white px-3 py-2 text-xs font-semibold">Tải nhiều ảnh cây<input type="file" accept="image/*" multiple className="hidden" onChange={e=>{const files=Array.from(e.target.files||[]);if(files.length)setRollFiles(current=>({...current,[i]:[...(current[i]||[]),...files]}))}}/></label><span className="text-xs text-neutral-500">{rollFiles[i]?.length?`Đã chọn thêm ${rollFiles[i].length} ảnh`:(r.images?.length?`${r.images.length} ảnh đã lưu`:"Chưa có ảnh")}</span>{r.images?.slice(0,4).map(img=><img key={img.id} src={assetUrl(img.url)} className="h-10 w-10 rounded-lg object-cover"/>)}</div>}</div>)}</div></div>
