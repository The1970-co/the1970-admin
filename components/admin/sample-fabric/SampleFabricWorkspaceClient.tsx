...
type Sample = {
  ...
  supplierId?: string | null; supplier?: Supplier | null; fabricBoardCode?: string | null; fabricCode?: string | null;
...
export default function SampleFabricWorkspaceClient({ defaultSection }: { defaultSection: Section }) {
  ...
  async function loadSamples() {
    const [rows, meta] = await Promise.all([
      ...
      api<{ suppliers: Supplier[]; staff: Staff[] }>("/sample-fabric/samples/meta"),
    ]);
    setSamples(rows); setSuppliers(meta.suppliers); setStaff(meta.staff);
  }
  ...
  const filteredSamples = useMemo(() => {
    ...
    return samples.filter(x => [x.code,x.name,x.category,x.fabricBoardCode,x.fabricCode,x.supplier?.name,x.assigneeName,...x.colors.map(c=>c.name)].some(v => String(v||"").toLowerCase().includes(key)));
  }, [samples,q]);
  ...
  return <div className="space-y-5 p-4 md:p-6">
    ...
    {showSampleForm && <SampleForm sample={editingSample} suppliers={suppliers} staff={staff} canUpload={can("design_sample.upload_images")} onClose={()=>setShowSampleForm(false)} onSaved={async()=>{setShowSampleForm(false);await reload()}} />}
    ...
    <div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">{rows.map(row=><Card key={row.id} className="overflow-hidden">
      <div className="flex gap-4 p-4">
        ...
        <div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><div><div className="text-xs font-semibold text-neutral-400">{row.code} · {row.year}</div><div className="mt-1 text-lg font-semibold text-neutral-950">{row.name}</div></div><Badge status={row.status}>{statusLabel(row.status,SAMPLE_STATUSES)}</Badge></div>
          <div className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1 text-xs text-neutral-600"><div>NCC: <b>{row.supplier?.name || "—"}</b></div><div>Bảng vải: <b>{row.fabricBoardCode || "—"}</b></div><div>Mã vải: <b>{row.fabricCode || "—"}</b></div><div>Phụ trách: <b>{row.assigneeName || "—"}</b></div></div>
        </div>
      </div>
...
function SampleForm({ sample, suppliers, staff, canUpload, onClose, onSaved }: { sample:Sample|null; suppliers:Supplier[]; staff:Staff[]; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
  const [form,setForm]=useState<any>({ name:sample?.name||"",code:sample?.code||"",year:sample?.year||new Date().getFullYear(),season:sample?.season||"",category:sample?.category||"",supplierId:sample?.supplierId||"",fabricBoardCode:sample?.fabricBoardCode||"",fabricCode:sample?.fabricCode||"",status:sample?.status||"IDEA",assigneeStaffId:sample?.assigneeStaffId||"",assigneeName:sample?.assigneeName||"",nextAction:sample?.nextAction||"",dueDate:sample?.dueDate?sample.dueDate.slice(0,10):"",note:sample?.note||"",technicalNote:sample?.technicalNote||"",coverImageUrl:sample?.coverImageUrl||""});
  const [colors,setColors]=useState<SampleColor[]>(sample?.colors?.length?sample.colors:[{name:"",code:"",status:sample?.status||"IDEA"}]);
  const [saving,setSaving]=useState(false); const [error,setError]=useState("");
  const patch=(k:string,v:any)=>setForm((x:any)=>({...x,[k]:v}));
  async function upload(file:File){const fd=new FormData();fd.append("file",file);const r=await api<{url:string}>("/sample-fabric/samples/upload",{method:"POST",body:fd});patch("coverImageUrl",r.url);}
  async function save(){try{setSaving(true);setError("");const assigned=staff.find(x=>x.id===form.assigneeStaffId);const payload={...form,assigneeName:assigned?.name||form.assigneeName||null,colors:colors.filter(x=>x.name.trim()).map(x=>({...x,status:x.status||form.status})),images:form.coverImageUrl?[{url:form.coverImageUrl,caption:"Ảnh đại diện"}]:[]};await api(sample?`/sample-fabric/samples/${sample.id}`:"/sample-fabric/samples",{method:sample?"PATCH":"POST",body:JSON.stringify(payload)});onSaved();}catch(e){setError(e instanceof Error?e.message:"Không lưu được mẫu.")}finally{setSaving(false)}}
  return <Modal title={sample?`Sửa mẫu ${sample.code}`:"Tạo mẫu mới"} onClose={onClose} wide><div className="space-y-5 p-5">{error&&<div className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}<div className="grid gap-4 md:grid-cols-3"><Field label="Tên mẫu"><input className={inputClass} value={form.name} onChange={e=>patch("name",e.target.value)}/></Field><Field label="Mã mẫu"><input className={inputClass} value={form.code} disabled={!!sample} placeholder="Tự sinh nếu bỏ trống" onChange={e=>patch("code",e.target.value)}/></Field><Field label="Năm"><input type="number" className={inputClass} value={form.year} onChange={e=>patch("year",e.target.value)}/></Field><Field label="Mùa / BST"><input className={inputClass} value={form.season} onChange={e=>patch("season",e.target.value)}/></Field><Field label="Nhóm sản phẩm"><input className={inputClass} value={form.category} onChange={e=>patch("category",e.target.value)}/></Field><Field label="Nhà cung cấp"><select className={inputClass} value={form.supplierId} onChange={e=>patch("supplierId",e.target.value)}><option value="">Chưa chọn</option>{suppliers.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></Field><Field label="Mã bảng vải"><input className={inputClass} value={form.fabricBoardCode} onChange={e=>patch("fabricBoardCode",e.target.value)}/></Field><Field label="Mã chất vải"><input className={inputClass} value={form.fabricCode} onChange={e=>patch("fabricCode",e.target.value)}/></Field><Field label="Tiến độ"><select className={inputClass} value={form.status} onChange={e=>patch("status",e.target.value)}>{SAMPLE_STATUSES.map(x=><option key={x[0]} value={x[0]}>{x[1]}</option>)}</select></Field><Field label="Người phụ trách"><select className={inputClass} value={form.assigneeStaffId} onChange={e=>patch("assigneeStaffId",e.target.value)}><option value="">Chưa gán</option>{staff.map(x=><option key={x.id} value={x.id}>{x.name} · {x.code}</option>)}</select></Field><Field label="Việc tiếp theo"><input className={inputClass} value={form.nextAction} onChange={e=>patch("nextAction",e.target.value)}/></Field><Field label="Hạn dự kiến"><input type="date" className={inputClass} value={form.dueDate} onChange={e=>patch("dueDate",e.target.value)}/></Field></div>
    <div><div className="mb-2 flex items-center justify-between"><b className="text-sm">Màu triển khai</b><button onClick={()=>setColors(x=>[...x,{name:"",code:"",status:form.status}])} className="rounded-xl border px-3 py-1.5 text-xs font-semibold">+ Thêm màu</button></div><div className="space-y-2">{colors.map((c,i)=><div key={i} className="grid gap-2 rounded-2xl bg-neutral-50 p-3 md:grid-cols-[1.2fr_1fr_1.2fr_auto]"><input className={inputClass} placeholder="Tên màu" value={c.name} onChange={e=>setColors(x=>x.map((v,j)=>j===i?{...v,name:e.target.value}:v))}/><input className={inputClass} placeholder="Mã màu" value={c.code||""} onChange={e=>setColors(x=>x.map((v,j)=>j===i?{...v,code:e.target.value}:v))}/><select className={inputClass} value={c.status||form.status} onChange={e=>setColors(x=>x.map((v,j)=>j===i?{...v,status:e.target.value}:v))}>{SAMPLE_STATUSES.map(x=><option key={x[0]} value={x[0]}>{x[1]}</option>)}</select><button onClick={()=>setColors(x=>x.filter((_,j)=>j!==i))} className="rounded-xl px-3 text-sm text-red-600">Xoá</button></div>)}</div></div>
    <div className="grid gap-4 md:grid-cols-2"><Field label="Ghi chú mẫu"><textarea className={`${inputClass} min-h-28`} value={form.note} onChange={e=>patch("note",e.target.value)}/></Field><Field label="Ghi chú kỹ thuật"><textarea className={`${inputClass} min-h-28`} value={form.technicalNote} onChange={e=>patch("technicalNote",e.target.value)}/></Field></div>
    <div className="rounded-2xl border border-dashed border-neutral-300 p-4"><div className="flex items-center gap-4">{form.coverImageUrl&&<img src={assetUrl(form.coverImageUrl)} className="h-24 w-20 rounded-xl object-cover"/>}<div><b className="text-sm">Ảnh mẫu / ảnh tham khảo</b>{canUpload?<input type="file" accept="image/*" className="mt-2 block text-xs" onChange={e=>e.target.files?.[0]&&upload(e.target.files[0])}/>:<div className="mt-1 text-xs text-neutral-400">Không có quyền upload ảnh</div>}</div></div></div>
    <div className="flex justify-end gap-2 border-t border-neutral-100 pt-4"><button onClick={onClose} className="rounded-2xl border px-4 py-2.5 text-sm">Đóng</button><button disabled={saving||!form.name.trim()} onClick={save} className="rounded-2xl bg-neutral-950 px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-40">{saving?"Đang lưu...":"Lưu mẫu"}</button></div></div></Modal>;
}
