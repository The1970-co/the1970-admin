...
type FabricReceipt = {
  ...
  fabricCosts?:FabricCostGroup[]; colorMaps?:FabricColorMap[]; costSummary?:ReceiptCostSummary|null;
};
...
function ReceiptForm({ receipt, suppliers, branches, staff, samples, products, boards, canFabricBoardLink, canSupplierIdentity, canCostView, canCostEdit, canUpload, onClose, onSaved }: { receipt:FabricReceipt|null; suppliers:Supplier[]; branches:Branch[]; staff:Staff[]; samples:any[]; products:Array<{id:string;name:string;slug:string;imageUrl?:string|null}>; boards:FabricBoard[]; canFabricBoardLink:boolean; canSupplierIdentity:boolean; canCostView:boolean; canCostEdit:boolean; canUpload:boolean; onClose:()=>void; onSaved:()=>void }) {
  ...
  const [sampleSource,setSampleSource]=useState<"PRODUCT"|"DESIGN">(()=>receipt?.productId?"PRODUCT":"DESIGN");
  ...
  const receiptFabricCodes=useMemo(()=>{
    const fromTop=parseFabricCodes(form.fabricCode);
    ...
    return Array.from(new Set([...fromTop,...fromMaps,...fromRolls]));
  },[form.fabricCode,colorMaps,rolls]);
  ...
  function chooseBoard(id:string){
    ...
    if(b.fabricCode){
      const merged=joinFabricCodes([...parseFabricCodes(form.fabricCode),...parseFabricCodes(b.fabricCode)]);
      patch("fabricCode",merged);
    }
    if(b.name&&!form.fabricName)patch("fabricName",b.name||"");
  ...
  function chooseSample(id:string){
    ...
    if(!sample||!canFabricBoardLink)return;
    const board=boards.find(x=>x.id===sample.fabricBoardId||x.boardCode===sample.fabricBoardCode||(
      !!sample.fabricCode&&parseFabricCodes(sample.fabricCode).some((code:string)=>parseFabricCodes(x.fabricCode).includes(code))
    ));
    if(board)chooseBoard(board.id);
    else{
      if(!form.fabricBoardCode&&sample.fabricBoardCode)patch("fabricBoardCode",String(sample.fabricBoardCode).replace(/[^A-Za-z0-9]/g,"").toUpperCase());
      if(sample.fabricCode)patch("fabricCode",joinFabricCodes([...parseFabricCodes(form.fabricCode),...parseFabricCodes(sample.fabricCode)]));
    }
  }

  function toggleBoardColor(c:BoardColor){
    const code=normalizeColorCode(String(c.code||c.name));
    const current=colorCodeList(form.colorCode);
    const next=current.includes(code)?current.filter(x=>x!==code):[...current,code];
    const names=next.map(x=>selectedBoard?.colors?.find(y=>normalizeColorCode(String(y.code||y.name))===x)?.name||"").filter(Boolean);
  ...
  return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose} wide><div className="space-y-5 p-5">{error&&<div className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>}
    <div className="grid gap-4 md:grid-cols-3">
      ...
      <Field label="Mẫu sử dụng"><div className="grid grid-cols-2 gap-2"><button type="button" onClick={()=>{setSampleSource("PRODUCT");patch("designSampleId","")}} className={`rounded-xl border px-3 py-2 text-xs font-semibold ${sampleSource==="PRODUCT"?"bg-neutral-950 text-white":"bg-white"}`}>Sản phẩm đã có</button><button type="button" onClick={()=>{setSampleSource("DESIGN");patch("productId","")}} className={`rounded-xl border px-3 py-2 text-xs font-semibold ${sampleSource==="DESIGN"?"bg-neutral-950 text-white":"bg-white"}`}>Mẫu triển khai</button></div>{sampleSource==="PRODUCT"?<select className={`${inputClass} mt-2`} value={form.productId||""} onChange={e=>patch("productId",e.target.value)}><option value="">Chưa chọn sản phẩm</option>{products.map(x=><option key={x.id} value={x.id}>{x.name} · {x.slug}</option>)}</select>:<select className={`${inputClass} mt-2`} value={form.designSampleId} onChange={e=>chooseSample(e.target.value)}><option value="">Chưa gắn mẫu triển khai</option>{samples.map(x=><option key={x.id} value={x.id}>{x.code} · {x.name}</option>)}</select>}</Field>
      ...
      <Field label="Mã vải"><input className={inputClass} value={form.fabricCode} onChange={e=>patch("fabricCode",e.target.value)} onBlur={()=>patch("fabricCode",joinFabricCodes(parseFabricCodes(form.fabricCode)))} placeholder="VD: AB88, AB99"/>{!!receiptFabricCodes.length&&<div className="mt-2 flex flex-wrap gap-1.5">{receiptFabricCodes.map(code=><span key={code} className="rounded-full bg-neutral-100 px-2 py-1 text-[11px] font-semibold">{code}</span>)}</div>}</Field>
      <Field label="Tên chất liệu vải"><input className={inputClass} value={form.fabricName} onChange={e=>patch("fabricName",e.target.value)}/></Field>
    ...
    <div className="rounded-3xl border p-4">
      <div className="flex items-start justify-between gap-3"><div><b className="text-sm">Cấu hình màu theo mã vải</b><div className="mt-1 text-xs text-neutral-400">VD: AB88 · Xám · #8; AB99 · Đen · #20; AB99 · Xanh · #21. Mã màu không bắt buộc.</div></div><button type="button" onClick={()=>addColorMap()} className="rounded-xl border px-3 py-2 text-xs font-semibold">+ Thêm màu</button></div>
      <div className="mt-3 space-y-2">{colorMaps.map((row,i)=><div key={row.id||i} className="grid gap-2 md:grid-cols-[160px_1fr_150px_auto]"><select className={inputClass} value={row.fabricCode||""} onChange={e=>patchColorMap(i,"fabricCode",e.target.value)}><option value="">Chọn mã vải</option>{receiptFabricCodes.map(code=><option key={code} value={code}>{code}</option>)}</select><input className={inputClass} value={row.colorName||""} onChange={e=>patchColorMap(i,"colorName",e.target.value)} placeholder="Tên màu · VD Đen"/><input className={inputClass} value={row.colorCode||""} onChange={e=>patchColorMap(i,"colorCode",e.target.value)} onBlur={()=>patchColorMap(i,"colorCode",normalizeColorCode(String(row.colorCode||"")))} placeholder="#20 · không bắt buộc"/><button type="button" onClick={()=>removeColorMap(i)} className="rounded-xl border px-3 text-xs font-semibold text-red-600">Xoá</button></div>)}</div>
      {!!colorMaps.length&&<button type="button" onClick={applyColorMapsToRolls} className="mt-3 rounded-xl border px-3 py-2 text-xs font-semibold">Áp cấu hình màu cho cây đã tạo</button>}
    </div>
