...
function ReceiptForm({receipt,meta,canFabricBoardLink,canSupplierIdentity,canCostView,canCostEdit,canUpload,onClose,onSaved}:{receipt:Receipt|null;meta:Meta;canFabricBoardLink:boolean;canSupplierIdentity:boolean;canCostView:boolean;canCostEdit:boolean;canUpload:boolean;onClose:()=>void;onSaved:()=>void}){
 ...
 const [fabricConfigs,setFabricConfigs]=useState<FabricConfig[]>(()=>{
  if(receipt?.fabricConfigs?.length)return receipt.fabricConfigs;
  const legacyCodes=Array.from(new Set([
   ...fabricCodeList(receipt?.fabricCode),
   ...(receipt?.colorMaps||[]).map(x=>String(x.fabricCode||"").trim().toUpperCase()).filter(Boolean),
   ...(receipt?.rolls||[]).map(x=>String(x.fabricCode||"").trim().toUpperCase()).filter(Boolean),
  ]));
  const rows=legacyCodes.map((code,i)=>({fabricCode:code,materialName:i===0?(receipt?.fabricName||""):"",supplierId:i===0?(receipt?.supplierId||null):null,fabricBoardCode:i===0?(receipt?.fabricBoardCode||null):null,fabricWidthCm:"",productId:null,designSampleId:i===0?(receipt?.designSampleId||null):null}));
 ...
 return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose}><div className="space-y-4 p-4">
  ...
  <section className="rounded-3xl border p-3">
   <div className="flex items-start justify-between gap-2">
    <div><b className="text-sm">Cấu hình theo mã vải</b><div className="mt-1 text-[11px] text-neutral-400">Mỗi mã vải có NCC, mã bảng vải, tên chất liệu, khổ vải, mẫu sử dụng và màu riêng.</div></div>
    <button type="button" onClick={()=>{setFabricConfigs(x=>[...x,{fabricCode:"",materialName:"",supplierId:null,fabricBoardCode:"",fabricWidthCm:"",productId:null,designSampleId:null}]);setConfigSource(x=>({...x,[fabricConfigs.length]:"DESIGN"}))}} className={smallBtn}>+ Mã vải</button>
   </div>
   <div className="mt-3 space-y-3">{fabricConfigs.map((cfg,ci)=>{const code=String(cfg.fabricCode||"").trim().toUpperCase();const source=configSource[ci]||(cfg.productId?"PRODUCT":"DESIGN");const colors=colorMaps.map((x,index)=>({x,index})).filter(({x})=>String(x.fabricCode||"").trim().toUpperCase()===code);return <div key={cfg.id||ci} className="rounded-3xl bg-neutral-50 p-3">
    <div className="mb-3 flex items-center justify-between"><div><div className="text-[10px] font-black uppercase tracking-wide text-neutral-400">Mã vải {ci+1}</div><b>{code||"Chưa nhập mã"}</b></div>{fabricConfigs.length>1&&<button type="button" onClick={()=>{setFabricConfigs(x=>x.filter((_,j)=>j!==ci));if(code)setColorMaps(x=>x.filter(c=>String(c.fabricCode||"").trim().toUpperCase()!==code))}} className="text-xs font-black text-red-500">Xoá mã</button>}</div>
    <div className="grid grid-cols-2 gap-2">
     <Field l="Mã vải"><input autoCapitalize="characters" className={input} value={cfg.fabricCode||""} onChange={e=>{const next=e.target.value.replace(/\s+/g,"").toUpperCase(),oldCode=String(cfg.fabricCode||"").trim().toUpperCase();setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,fabricCode:next}:v));if(oldCode&&next!==oldCode)setColorMaps(x=>x.map(c=>String(c.fabricCode||"").trim().toUpperCase()===oldCode?{...c,fabricCode:next}:c))}} placeholder="VD: AB99"/></Field>
     <Field l="Mã bảng vải"><input autoCapitalize="characters" className={input} value={cfg.fabricBoardCode||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,fabricBoardCode:e.target.value.replace(/[^A-Za-z0-9]/g,"").toUpperCase()}:v))} placeholder="VD: A2309"/></Field>
     <Field l="Tên chất liệu vải"><input className={input} value={cfg.materialName||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,materialName:e.target.value}:v))} placeholder="VD: Kaki co giãn"/></Field>
