...
type FabricCostGroup={id?:string;fabricCode:string;chinaShippingCny?:any;vietnamShippingVnd?:any;note?:string|null};
...
function ReceiptForm({receipt,meta,canFabricBoardLink,canSupplierIdentity,canCostView,canCostEdit,canUpload,onClose,onSaved}:{receipt:Receipt|null;meta:Meta;canFabricBoardLink:boolean;canSupplierIdentity:boolean;canCostView:boolean;canCostEdit:boolean;canUpload:boolean;onClose:()=>void;onSaved:()=>void}){
 ...
 const liveCostSummary=useMemo(()=>{const rate=num(f.exchangeRateToVnd),goodsCny=rolls.reduce((sum,r)=>sum+rollAmountCny(r),0),groups=fabricCodes.map(fabricCostFor),chinaShippingCny=groups.reduce((sum,x)=>sum+num(x.chinaShippingCny),0),vietnamShippingVnd=groups.reduce((sum,x)=>sum+num(x.vietnamShippingVnd),0),goodsVnd=goodsCny*rate,chinaShippingVnd=chinaShippingCny*rate,totalShippingVnd=chinaShippingVnd+vietnamShippingVnd;return{goodsCny,goodsVnd,chinaShippingCny,chinaShippingVnd,vietnamShippingVnd,totalShippingVnd,grandTotalVnd:goodsVnd+totalShippingVnd}},[rolls,fabricCosts,f.exchangeRateToVnd]);
 async function save(){
  try{
   setSaving(true);setError("");
   const receiver=meta.staff.find(x=>x.id===f.receivedByStaffId);const normalized=rolls.map((r,i)=>({...r,sortOrder:i+1,fabricCode:String(r.fabricCode||"").trim().toUpperCase()||null,colorCode:colorCode(r.colorCode)||null,unitPriceCny:canCostEdit?r.unitPriceCny:undefined,priceUnit:canCostEdit?(r.priceUnit||"METER"):undefined,defectNote:String(r.defectNote||"").trim()||null}));const normalizedFabricCosts=fabricCodes.map(code=>({...fabricCostFor(code),fabricCode:code,chinaShippingCny:num(fabricCostFor(code).chinaShippingCny),vietnamShippingVnd:num(fabricCostFor(code).vietnamShippingVnd)}));
 ...
 return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose}><div className="space-y-4 p-4">
  ...
  {canCostView&&<div className="rounded-3xl bg-amber-50 p-3"><Field l="Tỷ giá lúc nhập"><div className="relative"><input disabled={!canCostEdit} inputMode="decimal" className={`${input} pr-16 disabled:bg-neutral-100`} value={f.exchangeRateToVnd} onChange={e=>patch("exchangeRateToVnd",e.target.value)} placeholder="VD: 3.650"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-neutral-400">VND/CNY</span></div></Field><div className="mt-2 grid grid-cols-2 gap-2"><Mini l="Tiền vải" v={`${fmt(liveCostSummary.goodsCny,2)} CNY`}/><Mini l="Tổng đơn" v={money(liveCostSummary.grandTotalVnd)}/></div></div>}
  ...
  <section className="rounded-3xl border p-3">
   ...
   <div className="mt-3 space-y-3">{rolls.map((r,i)=><div key={r.id||i} className="rounded-3xl bg-neutral-50 p-3">
    ...
    </div>{canCostView&&<div className="mt-2 grid grid-cols-2 gap-2"><div className="relative"><input disabled={!canCostEdit} type="number" step="0.01" className={`${input} pr-12 disabled:bg-neutral-100`} value={r.unitPriceCny??""} onChange={e=>setRolls(x=>x.map((y,j)=>j===i?{...y,unitPriceCny:e.target.value}:y))} placeholder="Giá màu này"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400">CNY</span></div><select disabled={!canCostEdit} className={`${input} disabled:bg-neutral-100`} value={r.priceUnit||"METER"} onChange={e=>setRolls(x=>x.map((y,j)=>j===i?{...y,priceUnit:e.target.value as any}:y))}><option value="METER">Giá / mét</option><option value="KG">Giá / kg</option><option value="ROLL">Giá / cây</option></select><div className="col-span-2 rounded-2xl bg-white p-3 text-xs">Thành tiền <b>{fmt(rollAmountCny(r),2)} CNY</b>{num(f.exchangeRateToVnd)>0?<span className="ml-2 text-neutral-500">≈ {money(rollAmountCny(r)*num(f.exchangeRateToVnd))}</span>:null}</div></div>}
  ...
  {canCostView&&fabricCodes.length>0&&<section className="rounded-3xl border p-3"><b className="text-sm">Phí theo mã vải</b><div className="mt-2 space-y-2">{fabricCodes.map(code=>{const c=fabricCostFor(code);return <div key={code} className="rounded-2xl bg-neutral-50 p-3"><b>{code}</b><div className="mt-2 grid grid-cols-2 gap-2"><div className="relative"><input disabled={!canCostEdit} className={`${input} pr-12 disabled:bg-neutral-100`} value={c.chinaShippingCny??""} onChange={e=>patchFabricCost(code,"chinaShippingCny",e.target.value)} placeholder="Ship China"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400">CNY</span></div><div className="relative"><input disabled={!canCostEdit} className={`${input} pr-12 disabled:bg-neutral-100`} value={c.vietnamShippingVnd??""} onChange={e=>patchFabricCost(code,"vietnamShippingVnd",e.target.value)} placeholder="Ship VN"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400">VND</span></div></div><input disabled={!canCostEdit} className={`${input} mt-2 disabled:bg-neutral-100`} value={c.note||""} onChange={e=>patchFabricCost(code,"note",e.target.value)} placeholder="Ghi chú phí"/></div>})}</div><div className="mt-3 grid grid-cols-2 gap-2"><Mini l="Phí China" v={`${fmt(liveCostSummary.chinaShippingCny,2)} CNY`}/><Mini l="Phí Việt Nam" v={money(liveCostSummary.vietnamShippingVnd)}/><Mini l="Tổng phí" v={money(liveCostSummary.totalShippingVnd)}/><Mini l="Tổng đơn" v={money(liveCostSummary.grandTotalVnd)}/></div></section>}
