...
type Product={id:string;name:string;slug:string;imageUrl?:string|null};
type FabricConfig={id?:string;fabricCode:string;materialName?:string|null;supplierId?:string|null;fabricBoardCode?:string|null;fabricWidthCm?:any;productId?:string|null;designSampleId?:string|null};
type Roll={id?:string;sortOrder?:number|null;fabricCode?:string|null;rollCode?:string|null;colorName?:string|null;colorCode?:string|null;supplierDeclaredM?:any;supplierDeclaredKg?:any;actualM?:any;actualKg?:any;measuredGsm?:any;unitPriceCny?:any;priceUnit?:"METER"|"KG"|"ROLL"|null;lineAmountCny?:number|null;lineAmountVnd?:number|null;defectNote?:string|null;passed?:boolean;images?:Array<{id:string;url:string;caption?:string|null}>};
...
type Receipt={
 ...
 receivedAt?:string|null;completedAt?:string|null;receivedByStaffId?:string|null;receivedByName?:string|null;note?:string|null;rolls:Roll[];measurements:Measurement[];fabricCosts?:FabricCostGroup[];fabricConfigs?:FabricConfig[];colorMaps?:FabricColorMap[];costSummary?:ReceiptCostSummary|null;
...
type Meta={suppliers:Supplier[];branches:Branch[];staff:Staff[];samples:Sample[];products:Product[];boards:FabricBoard[]};
...
export default function Page(){
 ...
 const [meta,setMeta]=useState<Meta>({suppliers:[],branches:[],staff:[],samples:[],products:[],boards:[]});
 ...
 async function load(){
  try{
   ...
   setMeta({suppliers:Array.isArray(m.suppliers)?m.suppliers:[],branches:Array.isArray(m.branches)?m.branches:[],staff:Array.isArray((m as any).staff)?(m as any).staff:[],samples:Array.isArray(m.samples)?m.samples:[],products:Array.isArray((m as any).products)?(m as any).products:[],boards:Array.isArray((m as any).boards)?(m as any).boards:[]});
...
function ReceiptForm({receipt,meta,canFabricBoardLink,canSupplierIdentity,canCostView,canCostEdit,canUpload,onClose,onSaved}:{receipt:Receipt|null;meta:Meta;canFabricBoardLink:boolean;canSupplierIdentity:boolean;canCostView:boolean;canCostEdit:boolean;canUpload:boolean;onClose:()=>void;onSaved:()=>void}){
 ...
 const [fabricConfigs,setFabricConfigs]=useState<FabricConfig[]>(receipt?.fabricConfigs?.length?receipt.fabricConfigs:fabricCodeList(receipt?.fabricCode).map((code,i)=>({fabricCode:code,materialName:i===0?(receipt?.fabricName||""):"",supplierId:i===0?(receipt?.supplierId||null):null,fabricBoardCode:i===0?(receipt?.fabricBoardCode||null):null,fabricWidthCm:"",productId:null,designSampleId:i===0?(receipt?.designSampleId||null):null})));
 const [configSource,setConfigSource]=useState<Record<number,"PRODUCT"|"DESIGN">>(()=>Object.fromEntries((receipt?.fabricConfigs||[]).map((x,i)=>[i,x.productId?"PRODUCT":"DESIGN"])));
 ...
 const configuredFabricCodes=Array.from(new Set([
  ...fabricConfigs.map(x=>String(x.fabricCode||"").trim().toUpperCase()).filter(Boolean),
  ...topFabricCodes,
 ...
 return <Modal title={receipt?`Sửa ${receipt.receiptCode}`:"Tạo phiếu vải về"} onClose={onClose}><div className="space-y-4 p-4">
  ...
  <section className="rounded-3xl border p-3">
   ...
   <div className="mt-3 space-y-3">{fabricConfigs.map((cfg,ci)=>{const source=configSource[ci]||(cfg.productId?"PRODUCT":"DESIGN");return <div key={cfg.id||ci} className="rounded-2xl bg-neutral-50 p-2"><div className="grid grid-cols-2 gap-2"><input className={input} value={cfg.fabricCode||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,fabricCode:e.target.value.replace(/\s+/g,"").toUpperCase()}:v))} placeholder="Mã vải"/><input className={input} value={cfg.fabricBoardCode||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,fabricBoardCode:e.target.value.replace(/[^A-Za-z0-9]/g,"").toUpperCase()}:v))} placeholder="Mã bảng vải"/><input className={input} value={cfg.materialName||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,materialName:e.target.value}:v))} placeholder="Tên chất liệu vải"/><select className={input} value={cfg.supplierId||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,supplierId:e.target.value||null}:v))}><option value="">NCC</option>{meta.suppliers.map(sp=><option key={sp.id} value={sp.id}>{canSupplierIdentity?`${sp.code||"NCC"} · ${sp.name||""}`:(sp.code||"NCC")}</option>)}</select><input inputMode="decimal" className={input} value={decimalText(cfg.fabricWidthCm)} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,fabricWidthCm:decimalRaw(e.target.value)}:v))} placeholder="Khổ vải cm"/></div><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={()=>{setConfigSource(x=>({...x,[ci]:"PRODUCT"}));setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,designSampleId:null}:v))}} className={`rounded-xl border py-2 text-xs font-black ${source==="PRODUCT"?"bg-neutral-950 text-white":""}`}>Sản phẩm đã có</button><button type="button" onClick={()=>{setConfigSource(x=>({...x,[ci]:"DESIGN"}));setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,productId:null}:v))}} className={`rounded-xl border py-2 text-xs font-black ${source==="DESIGN"?"bg-neutral-950 text-white":""}`}>Mẫu triển khai</button></div>{source==="PRODUCT"?<select className={`${input} mt-2`} value={cfg.productId||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,productId:e.target.value||null}:v))}><option value="">Chọn sản phẩm</option>{meta.products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select>:<select className={`${input} mt-2`} value={cfg.designSampleId||""} onChange={e=>setFabricConfigs(x=>x.map((v,j)=>j===ci?{...v,designSampleId:e.target.value||null}:v))}><option value="">Chọn mẫu triển khai</option>{meta.samples.map(s=><option key={s.id} value={s.id}>{s.code} · {s.name}</option>)}</select>}</div>})}<button type="button" onClick={()=>{setFabricConfigs(x=>[...x,{fabricCode:"",materialName:"",supplierId:null,fabricBoardCode:"",fabricWidthCm:"",productId:null,designSampleId:null}]);setConfigSource(x=>({...x,[fabricConfigs.length]:"DESIGN"}))}} className="mt-2 w-full rounded-2xl border py-3 text-xs font-black">+ Thêm mã vải</button></div><div className="mt-3 space-y-2">{colorMaps.map((c,i)=><div key={c.id||i} className="rounded-2xl bg-neutral-50 p-2"><div className="grid grid-cols-2 gap-2"><input list="fabric-code-config-suggestions" autoCapitalize="characters" className={input} value={c.fabricCode||""} onChange={e=>patchColorMap(i,"fabricCode",e.target.value)} onBlur={()=>patchColorMap(i,"fabricCode",String(c.fabricCode||"").trim().toUpperCase())} placeholder="Mã vải · VD AB99"/><input className={input} value={c.colorName||""} onChange={e=>patchColorMap(i,"colorName",e.target.value)} placeholder="Tên màu · Đen"/><input className={input} value={c.colorCode||""} onChange={e=>patchColorMap(i,"colorCode",e.target.value)} onBlur={()=>patchColorMap(i,"colorCode",colorCode(c.colorCode))} placeholder="#20 · không bắt buộc"/><button type="button" onClick={()=>removeColorMap(i)} className="rounded-2xl border px-3 text-xs font-black text-red-500">Xoá</button></div></div>)}</div>
  ...
  <section className="rounded-3xl border p-3">
   ...
   {rollView==="TABLE"?<div className="mt-3 overflow-x-auto rounded-2xl border"><table className="min-w-[1180px] w-full text-[11px]"><thead className="bg-neutral-100"><tr>{["STT","Mã vải","Mã cây","Tên màu","Mã màu","NCC m","Thực kg","GSM","Khổ","Thực m","Giá CNY","Ghi chú"].map(h=><th key={h} className="px-2 py-2 text-left">{h}</th>)}</tr></thead><tbody>{rolls.map((r,i)=>{const cfg=fabricConfigs.find(c=>String(c.fabricCode||"").toUpperCase()===String(r.fabricCode||"").toUpperCase()),est=estimatedFabricMeters(r.actualKg,r.measuredGsm,cfg?.fabricWidthCm);return <tr key={r.id||i} className="border-t"><td className="px-2">{i+1}</td><td className="p-1"><select className="w-24 rounded-lg border p-2" value={r.fabricCode||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,fabricCode:e.target.value}:v))}><option value="">—</option>{configuredFabricCodes.map(c=><option key={c} value={c}>{c}</option>)}</select></td><td className="p-1"><input className="w-24 rounded-lg border p-2" value={r.rollCode||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,rollCode:e.target.value}:v))}/></td><td className="p-1"><input className="w-24 rounded-lg border p-2" value={r.colorName||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,colorName:e.target.value}:v))}/></td><td className="p-1"><input className="w-20 rounded-lg border p-2" value={r.colorCode||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,colorCode:e.target.value}:v))}/></td><td className="p-1"><input inputMode="decimal" className="w-20 rounded-lg border p-2" value={r.supplierDeclaredM??""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,supplierDeclaredM:e.target.value}:v))}/></td><td className="p-1"><input inputMode="decimal" className="w-20 rounded-lg border p-2" value={r.actualKg??""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,actualKg:e.target.value}:v))}/></td><td className="p-1"><input inputMode="decimal" className="w-20 rounded-lg border p-2" value={r.measuredGsm??""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,measuredGsm:e.target.value}:v))}/></td><td className="px-2">{cfg?.fabricWidthCm||"—"}</td><td className="p-1"><div className="flex gap-1"><input inputMode="decimal" className="w-20 rounded-lg border p-2" value={r.actualM??""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,actualM:e.target.value}:v))}/><button type="button" disabled={!est} onClick={()=>setRolls(x=>x.map((v,j)=>j===i?{...v,actualM:est.toFixed(3)}:v))} className="rounded-lg border px-2 disabled:opacity-30">≈m</button></div></td><td className="p-1">{canCostView?<input disabled={!canCostEdit} inputMode="decimal" className="w-20 rounded-lg border p-2" value={decimalText(r.unitPriceCny)} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,unitPriceCny:decimalRaw(e.target.value)}:v))}/>:null}</td><td className="p-1"><input className="w-40 rounded-lg border p-2" value={r.defectNote||""} onChange={e=>setRolls(x=>x.map((v,j)=>j===i?{...v,defectNote:e.target.value}:v))}/></td></tr>})}</tbody></table></div>:<div className="mt-3 space-y-3">{rolls.map((r,i)=><div key={r.id||i} className="rounded-3xl bg-neutral-50 p-3">
    ...
    <div className="mt-2 grid grid-cols-2 gap-2">
     ...
     <UnitInputBare unit="GSM" value={r.measuredGsm} onChange={v=>setRolls(x=>x.map((y,j)=>j===i?{...y,measuredGsm:v}:y))}/>
    <button type="button" onClick={()=>{const cfg=fabricConfigs.find(c=>String(c.fabricCode||"").toUpperCase()===String(r.fabricCode||"").toUpperCase()),est=estimatedFabricMeters(r.actualKg,r.measuredGsm,cfg?.fabricWidthCm);if(!est){alert("Cần Thực kg + GSM + Khổ vải (cm).");return}setRolls(x=>x.map((y,j)=>j===i?{...y,actualM:est.toFixed(3)}:y))}} className="col-span-2 rounded-2xl border bg-white py-3 text-xs font-black">Quy đổi kg + GSM → mét</button></div>{canCostView&&<div className="mt-2 grid grid-cols-2 gap-2"><div className="relative"><input disabled={!canCostEdit} inputMode="decimal" className={`${input} pr-12 disabled:bg-neutral-100`} value={decimalText(r.unitPriceCny)} onChange={e=>setRolls(x=>x.map((y,j)=>j===i?{...y,unitPriceCny:decimalRaw(e.target.value)}:y))} placeholder="Giá màu này"/><span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-neutral-400">CNY</span></div><select disabled={!canCostEdit} className={`${input} disabled:bg-neutral-100`} value={r.priceUnit||"METER"} onChange={e=>setRolls(x=>x.map((y,j)=>j===i?{...y,priceUnit:e.target.value as any}:y))}><option value="METER">Giá / mét</option><option value="KG">Giá / kg</option><option value="ROLL">Giá / cây</option></select><div className="col-span-2 rounded-2xl bg-white p-3 text-xs">Thành tiền <b>{fmt(rollAmountCny(r),2)} CNY</b>{num(f.exchangeRateToVnd)>0?<span className="ml-2 text-neutral-500">≈ {money(rollAmountCny(r)*num(f.exchangeRateToVnd))}</span>:null}</div></div>}
