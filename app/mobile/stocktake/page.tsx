...
const BarcodeScannerNative = registerPlugin<{
...
export default function MobileStocktakePage() {
    ...
    async function scanWithNativeCamera() {
        ...
        try {
            ...
            const result = await BarcodeScannerNative.scan();
            if (result?.cancelled) {
                setCameraMessage("Đã huỷ quét camera.");
                return true;
            }
            ...
            setCameraMessage(`Native scanner lỗi: ${message}. Kiểm tra đã thêm BarcodeScannerNativePlugin.swift/.m vào target App và build TestFlight mới.`);
            return true;
        }
    }
    ...
    async function startCamera() {
        ...
        if (!window.BarcodeDetector) {
            setCameraMessage(
                "Máy này chưa hỗ trợ quét barcode trực tiếp trong WebView. Cần build app iOS có BarcodeScannerNativePlugin.",
            );
            return;
        }
