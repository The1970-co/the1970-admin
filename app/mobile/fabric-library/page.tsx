...
export default function Page() {
    ...
    async function load() { try {
        ...
        setMeta({ suppliers: Array.isArray(m?.suppliers) ? m.suppliers : [], seasons: unique([...DEFAULT_SEASONS, ...(m?.seasons || [])]), productGroups: unique([...(groups || []), ...(m?.productGroups || [])]), fabricCompositions: unique([...DEFAULT_COMPOSITIONS, ...(m?.fabricCompositions || [])]), staff: m?.staff || [] });
    }
    catch (e) {
...
function BoardForm({ board, meta, canUpload, onClose, onSaved, onSupplierCreated }: {
    ...
    setCompositionDraft("");
    return;
} setCompositionParts(x => [...x, { name: clean, percent: "" }]); setCompositionDraft(""); } async function pickImage(file?: File) { if (!file)
    return; try {
    const r = await uploadBoardImage(file);
    setImages(x => [...x, { type: "BOARD", url: r.url }]);
    if (!form.coverImageUrl)
        patch("coverImageUrl", r.url);
}
catch (e) {
    setError(e instanceof Error ? e.message : "Không tải được ảnh.");
} } async function createSupplier() { if (!supplierName.trim())
    return; try {
...
function Modal({ title, children, onClose }: {
    ...
    children: any;
    onClose: () => void;
}) { return <div className="fixed inset-0 z-50 overflow-y-auto bg-black/45 p-3"><div className="mx-auto my-3 max-w-md overflow-hidden rounded-[30px] bg-white shadow-2xl"><div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white p-4"><h2 className="min-w-0 truncate pr-3 font-black">{title}</h2><button onClick={onClose} className="grid h-10 w-10 shrink-0 place-items-center rounded-full border"><X className="h-4 w-4"/></button></div>{children}</div></div>; }
